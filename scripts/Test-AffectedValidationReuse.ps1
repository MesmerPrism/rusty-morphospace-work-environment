...
try {
    ...
    $expectedJobs = [Collections.Generic.List[string]]::new(); [void]$expectedJobs.Add('infrastructure'); [void]$expectedJobs.Add('select')
    foreach ($platform in @('linux', 'windows')) {
        ...
        if ($platformChecks.Count -gt 0) {
            [void]$expectedJobs.Add("affected-$platform")
