...
function Assert-ReceiptRelativeArtifactBinding {
    $root=Join-Path ([IO.Path]::GetTempPath())('source-only-receipt-artifact-'+[guid]::NewGuid().ToString('N'))
    try{
        $f=New-Fixture $root 'fixture-receipt-artifact'
        [IO.File]::WriteAllText((Join-Path $f.workspace 'host.txt'),'pass',[Text.UTF8Encoding]::new($false))
        [IO.File]::WriteAllText((Join-Path $f.workspace 'receipts/host.txt'),'tampered',[Text.UTF8Encoding]::new($false))
        Assert-Rejected {Invoke-MorphospacePrepareSourceOnlyPublication -WorkspaceRoot $f.workspace -UnitId fixture-unit -RepoMapPath $f.map -SourceOnlyPublicationPlan $f.plan_path -ExpectedSourceOnlyPublicationPlanSha256 $f.plan_hash -OutPath (Join-Path $f.workspace 'receipts/fixture-receipt-artifact-plan.json') -Execute} 'receipt-relative artifact tamper behind workspace-root decoy' "*artifact 'host' drifted*" $f.workspace $f
