...
try{
    ...
    Assert-Retirement ($dry.proposed_retirement.PSObject.Properties.Name-cnotcontains'authenticated_ready_withdrawal') 'direct admission-tail retirement unexpectedly changed its binding shape'
    ...
    # C10B: an admitted proposal may be reviewed, withdrawn, and only then
    # retired.  The retirement owner must authenticate the complete contiguous
    # admission -> ordinary Ready -> WithdrawReady suffix.
    $admittedReadyRoot = Join-Path $temp 'admitted-ready-withdrawal'
    $admittedReady = New-RetirementAdmittedFixture $admittedReadyRoot
    $readyRun = Invoke-MorphospaceWorkUnitAutomation -Action Ready -WorkspaceRoot $admittedReady -UnitId u002 -Timestamp $timestamp -Execute
    Assert-Retirement ([string]$readyRun.status_after -ceq 'ready') 'admitted proposal did not pass ordinary Ready'
    $withdrawPath = Join-Path $admittedReady 'receipts/u002-ready-withdrawn.json'
    $withdrawRun = Invoke-MorphospaceWorkUnitAutomation -Action WithdrawReady -WorkspaceRoot $admittedReady -UnitId u002 -Timestamp $timestamp -OutPath $withdrawPath -Execute
    Assert-Retirement ([string]$withdrawRun.status_after -ceq 'proposed' -and (Test-Path $withdrawPath)) 'admitted proposal did not pass WithdrawReady'
    $admittedDry = Invoke-MorphospaceProposedUnitRetirement -WorkspaceRoot $admittedReady -UnitId u002 -ReplacementUnitId u003 -OutPath (Join-Path $admittedReady 'receipts/u002-admitted-retirement.json') -Timestamp $timestamp
    Assert-Retirement ($null -ne $admittedDry.proposed_retirement.authenticated_ready_withdrawal -and [string]$admittedDry.proposed_retirement.authenticated_ready_withdrawal.ready.event.event_id -like 'u002-ready-*') 'admitted Ready-to-WithdrawReady provenance was not included in retirement binding'
    Assert-Retirement (Test-Json -Json ($admittedDry | ConvertTo-Json -Depth 64 -Compress) -SchemaFile $narrowSchema) 'admitted provenance narrow dry run failed its closed schema'
    $admittedPreRetirement = Copy-RetirementFixture $admittedReady $temp 'admitted-ready-withdrawal-pre-retirement'
    $admittedExec = Get-RetirementExecutionArguments $admittedDry $admittedReady (Join-Path $admittedReady 'receipts/u002-admitted-retirement.json') $timestamp
    $admittedRun = Invoke-MorphospaceProposedUnitRetirement @admittedExec
    Assert-Retirement ([string]$admittedRun.status_after -ceq 'superseded') 'admitted Ready-to-WithdrawReady proposal did not retire'
    $tamperedAdmitted = Copy-RetirementFixture $admittedPreRetirement $temp 'admitted-ready-tampered-receipt'
    [IO.File]::AppendAllText((Join-Path $tamperedAdmitted 'receipts/u002-ready-withdrawn.json'), ' ', [Text.UTF8Encoding]::new($false))
    $tamperedRejected=$false;try{Invoke-MorphospaceProposedUnitRetirement -WorkspaceRoot $tamperedAdmitted -UnitId u002 -ReplacementUnitId u004 -OutPath (Join-Path $tamperedAdmitted 'receipts/u002-admitted-retirement.json') -Timestamp $timestamp|Out-Null}catch{$tamperedRejected=$true}
    Assert-Retirement $tamperedRejected 'admitted provenance accepted tampered withdrawal receipt bytes'
    $detachedReadyPathWorkspace=Copy-RetirementFixture $admittedPreRetirement $temp 'admitted-ready-detached-ready-path';$detachedReadyWithdrawalPath=Join-Path $detachedReadyPathWorkspace 'receipts/u002-ready-withdrawn.json';$detachedReadyWithdrawal=Read-EnvelopeProtocolJson $detachedReadyWithdrawalPath;$detachedReadyWithdrawal.ready_withdrawal.original_ready_transaction.intent.path='receipts/transactions/detached-ready-transition.intent.json';Write-EnvelopeJson $detachedReadyWithdrawalPath $detachedReadyWithdrawal
    $detachedReadyEvents=@(Get-Content -LiteralPath (Join-Path $detachedReadyPathWorkspace 'iteration-events.jsonl')|Where-Object{$_}|ForEach-Object{$_|ConvertFrom-Json -DateKind String});$detachedReadyWithdrawEvent=$detachedReadyEvents[-1];$detachedReadyWithdrawIntentPath=Join-Path $detachedReadyPathWorkspace "receipts/transactions/$([string]$detachedReadyWithdrawEvent.event_id)-transition.intent.json";$detachedReadyWithdrawIntent=Read-EnvelopeProtocolJson $detachedReadyWithdrawIntentPath;$detachedReadyWithdrawIntent.artifacts[0].sha256=Get-EnvelopeFileSha256 $detachedReadyWithdrawalPath;$detachedReadyWithdrawIntent.artifacts[0].bytes_base64=[Convert]::ToBase64String([IO.File]::ReadAllBytes($detachedReadyWithdrawalPath));Write-EnvelopeJson $detachedReadyWithdrawIntentPath $detachedReadyWithdrawIntent
    $detachedReadyWithdrawCompletionPath=$detachedReadyWithdrawIntentPath.Replace('.intent.json','.completion.json');$detachedReadyWithdrawCompletion=Read-EnvelopeProtocolJson $detachedReadyWithdrawCompletionPath;$detachedReadyWithdrawCompletion.intent.sha256=Get-EnvelopeFileSha256 $detachedReadyWithdrawIntentPath;Write-EnvelopeJson $detachedReadyWithdrawCompletionPath $detachedReadyWithdrawCompletion;$detachedReadyPathSnapshot=Get-RetirementInventory $detachedReadyPathWorkspace;$detachedReadyPathMessage=''
    try{Invoke-MorphospaceProposedUnitRetirement -WorkspaceRoot $detachedReadyPathWorkspace -UnitId u002 -ReplacementUnitId u003 -OutPath (Join-Path $detachedReadyPathWorkspace 'receipts/u002-admitted-retirement.json') -Timestamp $timestamp|Out-Null}catch{$detachedReadyPathMessage=$_.Exception.Message}
    Assert-Retirement ($detachedReadyPathMessage-like'*withdrawal receipt does not reproduce*'-and$detachedReadyPathSnapshot-ceq(Get-RetirementInventory $detachedReadyPathWorkspace)) "admitted provenance accepted or mutated a recomputed detached Ready transaction path (observed '$detachedReadyPathMessage')"
    $interveningAdmitted = Copy-RetirementFixture $admittedPreRetirement $temp 'admitted-ready-intervening'
    $interveningStatePath = Join-Path $interveningAdmitted 'workspace.state.json';$interveningState = Read-EnvelopeProtocolJson $interveningStatePath;$interveningEventsPath=Join-Path $interveningAdmitted 'iteration-events.jsonl';$interveningEvents=@(Get-Content $interveningEventsPath|Where-Object{$_}|ForEach-Object{$_|ConvertFrom-Json -DateKind String});$interveningId='u002-intervening-0005';$interveningEvent=[ordered]@{schema='rusty.morphospace.workflow.iteration_event.v1';event_id=$interveningId;sequence=($interveningEvents.Count+1);timestamp=$timestamp;project_id='envelope-test';unit_id='u002';event_type='decision';summary='Intervening event.';receipts=@()};[IO.File]::AppendAllText($interveningEventsPath,(($interveningEvent|ConvertTo-Json -Compress)+"`n"));$interveningState.last_event_id=$interveningId;Write-EnvelopeJson $interveningStatePath $interveningState;$interveningRejected=$false;try{Invoke-MorphospaceProposedUnitRetirement -WorkspaceRoot $interveningAdmitted -UnitId u002 -ReplacementUnitId u004 -OutPath (Join-Path $interveningAdmitted 'receipts/u002-admitted-retirement.json') -Timestamp $timestamp|Out-Null}catch{$interveningRejected=$true};Assert-Retirement $interveningRejected 'admitted provenance accepted an intervening event'
    ...
    $admittedRetirementEvent=@(Get-Content -LiteralPath (Join-Path $admittedReady 'iteration-events.jsonl')|Where-Object{$_}|ForEach-Object{$_|ConvertFrom-Json -DateKind String})[-1]
    $admittedRetirementStep=&$recoveryLedgerModule {param($root,$id)Test-MorphospaceCommittedTransitionLedger -WorkspaceRoot $root -TransactionId $id -ExpectedStatePath 'workspace.state.json' -ExpectedUnitPath 'iteration-units/u002.json' -ExpectedEventsPath 'iteration-events.jsonl'} $admittedReady "$([string]$admittedRetirementEvent.event_id)-transition"
    $admittedHistorical=&$compatibilityModule {param($root,$expected,$committed)Test-MorphospaceHistoricalProposedRetirement -WorkspaceRoot $root -ExpectedEvent $expected -CommittedStep $committed} $admittedReady $admittedRetirementEvent $admittedRetirementStep
    Assert-Retirement ([string]$admittedHistorical.unit_id-ceq'u002'-and[string]$admittedHistorical.replacement_unit_id-ceq'u003'-and-not[bool]$admittedHistorical.grants_validation_credit) 'current-work reader did not authenticate the admitted Ready-to-WithdrawReady retirement'
    $detachedArtifactWorkspace=Copy-RetirementFixture $admittedReady $temp 'admitted-ready-detached-withdrawal-artifact';$detachedRetirementPath=Join-Path $detachedArtifactWorkspace 'receipts/u002-admitted-retirement.json';$detachedReceipt=Read-EnvelopeProtocolJson $detachedRetirementPath;$detachedProvenance=$detachedReceipt.proposed_retirement.authenticated_ready_withdrawal
    $detachedWithdrawIntentPath=Join-Path $detachedArtifactWorkspace ([string]$detachedProvenance.withdrawal.transaction.intent.path);$detachedWithdrawIntent=Read-EnvelopeProtocolJson $detachedWithdrawIntentPath;$detachedWithdrawIntent.artifacts=@();Write-EnvelopeJson $detachedWithdrawIntentPath $detachedWithdrawIntent
    $detachedWithdrawCompletionPath=Join-Path $detachedArtifactWorkspace ([string]$detachedProvenance.withdrawal.transaction.completion.path);$detachedWithdrawCompletion=Read-EnvelopeProtocolJson $detachedWithdrawCompletionPath;$detachedWithdrawCompletion.intent.sha256=Get-EnvelopeFileSha256 $detachedWithdrawIntentPath;Write-EnvelopeJson $detachedWithdrawCompletionPath $detachedWithdrawCompletion
    $detachedProvenance.withdrawal.transaction.intent.sha256=Get-EnvelopeFileSha256 $detachedWithdrawIntentPath;$detachedProvenance.withdrawal.transaction.completion.sha256=Get-EnvelopeFileSha256 $detachedWithdrawCompletionPath;$detachedBinding=Copy-RetirementObject $detachedReceipt.proposed_retirement;$detachedBinding.PSObject.Properties.Remove('binding_sha256');$detachedReceipt.proposed_retirement.binding_sha256=Get-EnvelopeCanonicalJsonSha256 $detachedBinding;Write-EnvelopeJson $detachedRetirementPath $detachedReceipt
    $detachedEvent=@(Get-Content -LiteralPath (Join-Path $detachedArtifactWorkspace 'iteration-events.jsonl')|Where-Object{$_}|ForEach-Object{$_|ConvertFrom-Json -DateKind String})[-1];$detachedRetirementIntentPath=Join-Path $detachedArtifactWorkspace "receipts/transactions/$([string]$detachedEvent.event_id)-transition.intent.json";$detachedRetirementIntent=Read-EnvelopeProtocolJson $detachedRetirementIntentPath;$detachedRetirementIntent.artifacts[0].sha256=Get-EnvelopeFileSha256 $detachedRetirementPath;$detachedRetirementIntent.artifacts[0].bytes_base64=[Convert]::ToBase64String([IO.File]::ReadAllBytes($detachedRetirementPath));Write-EnvelopeJson $detachedRetirementIntentPath $detachedRetirementIntent
    $detachedRetirementCompletionPath=$detachedRetirementIntentPath.Replace('.intent.json','.completion.json');$detachedRetirementCompletion=Read-EnvelopeProtocolJson $detachedRetirementCompletionPath;$detachedRetirementCompletion.intent.sha256=Get-EnvelopeFileSha256 $detachedRetirementIntentPath;Write-EnvelopeJson $detachedRetirementCompletionPath $detachedRetirementCompletion;$detachedMessage=''
    try{$detachedStep=&$recoveryLedgerModule {param($root,$id)Test-MorphospaceCommittedTransitionLedger -WorkspaceRoot $root -TransactionId $id -ExpectedStatePath 'workspace.state.json' -ExpectedUnitPath 'iteration-units/u002.json' -ExpectedEventsPath 'iteration-events.jsonl'} $detachedArtifactWorkspace "$([string]$detachedEvent.event_id)-transition";&$compatibilityModule {param($root,$expected,$committed)Test-MorphospaceHistoricalProposedRetirement -WorkspaceRoot $root -ExpectedEvent $expected -CommittedStep $committed|Out-Null} $detachedArtifactWorkspace $detachedEvent $detachedStep}catch{$detachedMessage=$_.Exception.Message}
    Assert-Retirement ($detachedMessage-like'*artifact ownership is detached*') "current-work reader accepted a recomputed withdrawal transaction without receipt ownership (observed '$detachedMessage')"
