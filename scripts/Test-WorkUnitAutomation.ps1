...
try {
    ...
    # RetireProposed existed on this broad automation surface before its typed-owner
    ...
    $legacyRetirementCliDry=& (Join-Path $PSScriptRoot 'Invoke-WorkUnitAutomation.ps1') @legacyRetirementBase -WorkspaceRoot $legacyRetirementCliWorkspace -OutPath $legacyRetirementCliOut|ConvertFrom-Json -Depth 64 -DateKind String
    Assert-Automation ([string]$legacyRetirementModuleDry.timestamp-ceq$legacyRetirementTimestamp-and(Get-TestCanonicalHash $legacyRetirementModuleDry)-ceq(Get-TestCanonicalHash $legacyRetirementCliDry)) 'RetireProposed changed pre-refactor legacy timestamp spelling or module/CLI dry output'
    function Get-LegacyRetirementExecution([object]$Dry,[string]$Workspace,[string]$OutPath){$pre=$Dry.proposed_retirement.authenticated_preimage;@{Action='RetireProposed';WorkspaceRoot=$Workspace;UnitId='u002';ReplacementUnitId='u003';RetirementReason='contract-invalid';Timestamp=$legacyRetirementTimestamp;OutPath=$OutPath;ExpectedStateSha256=[string]$pre.state_sha256;ExpectedUnitSha256=[string]$pre.unit_sha256;ExpectedUnitRawSha256=[string]$pre.unit_raw_sha256;ExpectedEventsSha256=[string]$pre.events_sha256;ExpectedEventsLength=[long]$pre.events_length;ExpectedEventTailId=[string]$pre.event_tail_id;ExpectedProposedRetirementBindingSha256=[string]$Dry.proposed_retirement.binding_sha256;Execute=$true}}
    $legacyRetirementModuleExecution=Get-LegacyRetirementExecution $legacyRetirementModuleDry $legacyRetirementModuleWorkspace $legacyRetirementModuleOut;$legacyRetirementCliExecution=Get-LegacyRetirementExecution $legacyRetirementCliDry $legacyRetirementCliWorkspace $legacyRetirementCliOut
    $legacyRetirementModuleRun=Invoke-MorphospaceWorkUnitAutomation @legacyRetirementModuleExecution
    $legacyRetirementCliRun=& (Join-Path $PSScriptRoot 'Invoke-WorkUnitAutomation.ps1') @legacyRetirementCliExecution|ConvertFrom-Json -Depth 64 -DateKind String
    Assert-Automation ((Get-TestCanonicalHash $legacyRetirementModuleRun)-ceq(Get-TestCanonicalHash $legacyRetirementCliRun)-and[Convert]::ToBase64String([IO.File]::ReadAllBytes($legacyRetirementModuleOut))-ceq[Convert]::ToBase64String([IO.File]::ReadAllBytes($legacyRetirementCliOut))-and[string]$legacyRetirementModuleRun.timestamp-ceq$legacyRetirementTimestamp) 'RetireProposed legacy module/CLI result or receipt bytes changed'
    $legacyRetirementEvent=@(Get-Content -LiteralPath (Join-Path $legacyRetirementModuleWorkspace 'iteration-events.jsonl')|Where-Object{$_}|ForEach-Object{$_|ConvertFrom-Json -DateKind String})[-1]
    Assert-Automation ([string]$legacyRetirementEvent.timestamp-ceq$legacyRetirementTimestamp) 'RetireProposed changed the pre-refactor legacy event timestamp spelling'
    $legacyTransactionDocuments=@()
    foreach($row in @(@{workspace=$legacyRetirementModuleWorkspace;result=$legacyRetirementModuleRun;receipt=$legacyRetirementModuleOut},@{workspace=$legacyRetirementCliWorkspace;result=$legacyRetirementCliRun;receipt=$legacyRetirementCliOut})){$id="$([string]$row.result.event_id)-transition";$intentPath=Join-Path $row.workspace "receipts/transactions/$id.intent.json";$completionPath=Join-Path $row.workspace "receipts/transactions/$id.completion.json";$intent=Read-TestProtocolJson $intentPath;$completion=Read-TestProtocolJson $completionPath;Assert-Automation ([string]$completion.intent.sha256-ceq(Get-TestFileHash $intentPath)-and[string]$intent.artifacts[0].sha256-ceq(Get-TestFileHash $row.receipt)) 'RetireProposed legacy transaction intent/completion binding is invalid';$legacyTransactionDocuments+=,[pscustomobject]@{intent=$intent;completion=$completion}}
    $normalizedLegacyIntents=@($legacyTransactionDocuments|ForEach-Object{$copy=$_.intent|ConvertTo-Json -Depth 64|ConvertFrom-Json -Depth 64 -DateKind String;$copy.created_at='<owner-clock>';$copy});$normalizedLegacyCompletions=@($legacyTransactionDocuments|ForEach-Object{$copy=$_.completion|ConvertTo-Json -Depth 64|ConvertFrom-Json -Depth 64 -DateKind String;$copy.completed_at='<owner-clock>';$copy.intent.sha256='0'*64;$copy})
    Assert-Automation ((Get-TestCanonicalHash $normalizedLegacyIntents[0])-ceq(Get-TestCanonicalHash $normalizedLegacyIntents[1])-and(Get-TestCanonicalHash $normalizedLegacyCompletions[0])-ceq(Get-TestCanonicalHash $normalizedLegacyCompletions[1])) 'RetireProposed legacy transaction structure differs beyond ledger-owned timestamps and authenticated intent-hash role'
