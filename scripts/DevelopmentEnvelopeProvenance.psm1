...
function Test-MorphospacePreparedDevelopmentEnvelope {
 ...
 if($recovered){$recoveryArtifact=@($artifacts|Where-Object{[string]$_.path-ceq[string]$p.recovery_receipt_path});if($recoveryArtifact.Count-ne1-or[string]$recoveryArtifact[0].sha256-cne(Get-MorphospaceCanonicalJsonSha256 $recoveryReceipt)-or[string]$completion.repreparation_receipt_sha256-cne[string]$recoveryArtifact[0].sha256){throw 'Recovered prepared-envelope transaction does not bind its exact recovery receipt.'}}
