...
'@
    ...
    if ($runFullSelector -or $runExecutorPassPhase -or $runExecutorDamagePhase) {
        ...
        if ($runFullSelector -or $runExecutorPassPhase) {
            ...
            $inconsistentLeafFailureKind = $documentationReceiptValue | ConvertTo-Json -Depth 64 | ConvertFrom-Json -Depth 64 -DateKind String
            $inconsistentLeafFailureKind.child.failure_kind = 'exit-code'
            Assert-True (-not (Test-Json -Json (ConvertTo-MorphospaceCanonicalJson -Value $inconsistentLeafFailureKind) -SchemaFile (Join-Path $repoRoot 'schemas/affected-validation-check-evidence-v1.schema.json') -ErrorAction SilentlyContinue)) 'Leaf evidence schema accepted inconsistent typed failure data.'
            ...
            $drainCodeFailure.result = 'code-fail'; $drainCodeFailure.check_results[0].result = 'code-fail'; $drainCodeFailure.check_results[0].failure_kind='drain-timeout'; $drainCodeFailure.check_results[0].exit_code = $null; $drainCodeFailure.check_results[0].post_kill_drain_timed_out = $true
            ...
            $mixedFailure.result = 'code-fail'; $mixedFailure.check_results[0].result = 'code-fail'; $mixedFailure.check_results[0].failure_kind='exit-code'; $mixedFailure.check_results[0].exit_code = 1; $mixedFailure.check_results[1].result = 'infra-fail'; $mixedFailure.check_results[1].failure_kind='launch'; $mixedFailure.check_results[1].exit_code = $null
        ...
        foreach ($platform in @('linux','windows')) {
            ...
            foreach ($segment in $platformSegments) {
                ...
                foreach ($id in @($segment.check_ids)) {
                    ...
                    $segmentResults.Add([pscustomobject][ordered]@{check_id=[string]$id;command_path=$commandPath;command_blob_sha1=[string]$entry.blob;result='pass';failure_kind=$null;exit_code=0;timed_out=$false;output_truncated=$false;post_kill_drain_timed_out=$false;stdout_sha256=('0'*64);stderr_sha256=('0'*64);stdout_bytes=0;stderr_bytes=0})
                }
