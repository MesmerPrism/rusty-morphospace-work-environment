...
function Assert-AffectedExecutorContainmentSource([string]$Source) {
    Assert-True ($Source -notmatch 'TryReadControl|ControlPath|leaf\.stdout\.bin|leaf\.stderr\.bin|ReadAllBytes\(\$stdoutPath\)|disableMaxPrivilege|0x000f0fff|CreateRestrictedLeafToken|RUSTY_AFFECTED_VALIDATION_RESTRICTING_SIDS|ProcessStartInfo\]\:\:new\(\$unshare\.Source\)' -and $Source -match 'AnonymousPipeServerStream' -and $Source -match 'ApplySupervisorCompletion' -and $Source -match 'W017SupervisorProtection' -and $Source -match 'ProtectAncestors' -and $Source -match 'ProtectThreads' -and $Source -match 'SnapshotAncestorSecurity' -and $Source -match 'RestoreProtectedFutureThreads' -and $Source -match 'RetainFutureThread' -and $Source -match 'retainedThreadIds' -and $Source -match 'restoredThreadHandles' -and $Source -match 'RunRestorationCollisionSelfTests' -and $Source -match 'stable residue-free thread set' -and $Source -match 'trusted ancestor fallback process/thread restoration readback differs' -and $Source -match 'trusted ancestor process/thread owner/DACL restoration readback differs' -and $Source -match 'token-default-DACL restoration readback differs' -and $Source -match 'RCWDWO;;;OW' -and $Source -match 'RUSTY_AFFECTED_VALIDATION_GUARD_SID' -and $Source -match 'guard-disabled leaf token did not retain the authority group as deny-only' -and $Source -match '0x1000' -and $Source -match '0x2000' -and $Source -match 'OwnerSecurityInformation\|DaclSecurityInformation' -and $Source -match 'CreateRestrictedToken' -and $Source -match 'CreatePrivilegeStrippedToken' -and $Source -match 'SetTokenDefaultDacl' -and $Source -match 'OpenThread' -and $Source -match 'Thread32First' -and $Source -match 'RUSTY_AFFECTED_VALIDATION_FUTURE_THREAD_ID' -and $Source -match 'RUSTY_AFFECTED_VALIDATION_PARENT_FUTURE_THREAD_ID' -and $Source -match 'ReadPrivileges' -and $Source -match 'RUSTY_AFFECTED_VALIDATION_REMOVED_PRIVILEGE_LUID' -and $Source -match 'privilege-deleted leaf token retained a non-allowlisted privilege' -and $Source -match 'AssertAncestorIsolation' -and $Source -match 'ImpersonateLoggedOnUser' -and $Source -match 'exact two-process completion authority chain' -and $Source -match 'Initialize-LeafWritableRoot' -and $Source -match 'SetKernelObjectSecurity' -and $Source -match 'W017SupervisorInnerJob' -and $Source -match 'CreatePipe' -and $Source -match 'TerminateJobObject\(state\.job,1\)' -and $Source -match "Get-Command unshare" -and $Source -match '\[Array\]\:\:Sort\(\$orderedUnsharePaths,\[StringComparer\]\:\:Ordinal\)' -and $Source -match 'ProcessStartInfo\]\:\:new\(\$unsharePath\)' -and $Source -match "--pid','--fork','--kill-child=KILL" -and $Source -match 'Complete-Pump' -and $Source -match 'process\.Kill\(true\)' -and $Source -match 'RunForSetupFailureTest') 'Affected executor does not close guard-SID, owner-rights, process/thread/token-query rights including post-probe threads, privilege deletion, token-default protection/restoration, deterministic Linux launcher resolution, exact process/thread readback, private completion, live-output bounds, namespace escape, complete drain, or setup-fallback containment.'
}
...
'@
    ...
    if ($runFullSelector -or $runExecutorPassPhase -or $runExecutorDamagePhase) {
        ...
        if ($runFullSelector -or $runExecutorPassPhase) {
            ...
            $immutableFailureReceipt.mode = 'executed'; $immutableFailureReceipt.result = 'code-fail'; $immutableFailureReceipt.child.exit_code = 17; $immutableFailureReceipt.artifacts = @(); $immutableFailureReceipt.reused_from = $null
            ...
            $savedMutatePrior = [Environment]::GetEnvironmentVariable('RUSTY_TEST_MUTATE_PRIOR','Process')
            try {
                [Environment]::SetEnvironmentVariable('RUSTY_TEST_MUTATE_PRIOR',$snapshotDocumentationReceipt[0].FullName,'Process')
            ...
            } finally { [Environment]::SetEnvironmentVariable('RUSTY_TEST_MUTATE_PRIOR',$savedMutatePrior,'Process') }
