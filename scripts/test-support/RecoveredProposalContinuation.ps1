...
function Test-RecoveredProposalRetirement {
    ...
    Get-ChildItem -LiteralPath $workspace -Force|Copy-Item -Destination $repositoryWorkspace -Recurse -Force;Invoke-EnvelopeGit $repositoryRoot @('add','morphospace')|Out-Null;Invoke-EnvelopeGit $repositoryRoot @('commit','-m','fixture recovered retirement checkpoint')|Out-Null
