...
function Test-RecoveredProposalRetirement {
    ...
    Assert-Envelope ($publicDamage.exit_code-ne0-and$publicDamage.output-clike'*Malformed admission completion raw or canonical SHA-256 differs from its exact binding.*'-and$publicDamageBefore-ceq(Get-EnvelopeWorkspaceByteInventorySha256 $publicDamageWorkspace)) "public current-work validation did not reject a damaged retained completion at its exact evidence-hash predicate or mutated the workspace:`n$($publicDamage.output)"
