...
$trustMigration = Join-Path $PSScriptRoot 'Test-TrustMigrationAuthority.ps1'
...
$trustResult = Invoke-ValidatorCommand -CommandId 'trust-migration-authority-self-test' -ScriptPath $trustMigration -Arguments @()
if ($executionResult.exit_code -ne 0 -or $transitionResult.exit_code -ne 0 -or $handoffResult.exit_code -ne 0 -or $trustResult.exit_code -ne 0) { throw 'Authority execution, runner handoff, trust migration, or transition-ledger self-test failed.' }
$criterionCommands = @{
