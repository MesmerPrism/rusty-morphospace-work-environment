...
function Invoke-ExactGit { param([string]$Root,[string[]]$Arguments) $out=@(& git -C $Root @Arguments 2>&1|ForEach-Object{[string]$_});if($LASTEXITCODE-ne0){throw "Git query failed in '$Root': git $($Arguments -join ' ')"};return @($out) }

$workspace = [IO.Path]::GetFullPath($WorkspaceRoot)
$spec = Read-MorphospaceProtocolJson -Path (Join-Path $workspace "project.spec.json")
$unit = Read-MorphospaceProtocolJson -Path (Join-Path $workspace (Join-Path "iteration-units" "$UnitId.json"))
$map = Read-MorphospaceProtocolJson -Path $RepositoryMapPath
...
foreach($entry in @($map.repositories|Sort-Object repo_id)){
    ...
    $status=@(Invoke-ExactGit $root @("status","--porcelain=v1","--untracked-files=no"));if($status.Count-gt0){throw "Source composition repository has tracked changes: $id"}
    ...
    $remote=@(& git -C $root remote get-url origin 2>$null);$remoteUrl=if($LASTEXITCODE-eq0-and$remote.Count-gt0){([string]$remote[0]).Trim()}else{$null}
...
$fingerprint=Get-MorphospaceCanonicalJsonSha256 -Value ([pscustomobject][ordered]@{schema="rusty.morphospace.workflow.source_composition_identity.v1";project_id=[string]$spec.project_id;unit_id=$UnitId;repositories=@($records.ToArray())})
...
Write-MorphospaceManagedProtocolJsonAtomic -WorkspaceRoot $workspace -RelativePath $OutRelativePath -Value $lock -NoOverwrite
