...
function Get-PreparationProtocolBytes { param([object]$Value) ConvertTo-MorphospaceProtocolJsonBytes $Value }
function Get-PreparationByteHash { param([byte[]]$Bytes) Get-MorphospaceSha256Bytes $Bytes }
function Test-PreparationHasToolingContext { param([object]$Value) $null-ne$Value-and$Value.PSObject.Properties.Name-ccontains'tooling_context' }
function Test-PreparationHasToolingReclassification { param([object]$Value) $null-ne$Value-and$Value.PSObject.Properties.Name-ccontains'legacy_tooling_reclassification' }
function Get-PreparationLegacyToolingHistory {
    param([string]$Workspace,[string]$PreparationEventId)
    $events=@(Get-Content -LiteralPath (Get-PreparationPath $Workspace 'iteration-events.jsonl')|Where-Object{$_}|ForEach-Object{$_|ConvertFrom-Json -DateKind String});Get-MorphospaceLegacyToolingReclassificationHistory -WorkspaceRoot $Workspace -Events $events -PreparationEventId $PreparationEventId
}
...
function Assert-PreparationAdditiveProject {
    ...
    Assert-MorphospaceDevelopmentEnvelopeAdditiveProject $Current $Target $AllowSchemaPinAdvance $OwnerRepositories $Mode
}
...
function Assert-PreparationEnvelope {
    ...
    Assert-MorphospaceDevelopmentEnvelope $Preparation $Project $FeatureLock $Mode
}
function Assert-PreparationToolingContextArtifact {
    param([string]$Workspace,[string]$RepoRoot,[object]$Receipt,[object]$Intent,[object]$Source,[byte[]]$SourceBytes,[object]$Context,[byte[]]$ContextBytes,[Management.Automation.PSModuleInfo]$OwnerModule)
    if(-not(Test-PreparationHasToolingContext $Receipt)-or-not(Test-PreparationHasToolingContext $Intent)){throw 'Preparation tooling context is not bound by both receipt and intent.'}
    if((Get-PreparationHash $Receipt.tooling_context)-cne(Get-PreparationHash $Intent.tooling_context)){throw 'Preparation receipt and intent tooling-context pointers differ.'}
    $pointer=$Receipt.tooling_context
    if([string]$pointer.sha256-cne(Get-PreparationByteHash $ContextBytes)-or[string]$pointer.canonical_sha256-cne(Get-PreparationHash $Context)){throw 'Preparation tooling-context pointer does not authenticate exact raw and canonical context bytes.'}
    if([string]$Context.project_id-cne[string]$Receipt.project_id-or[string]$Context.preparation_id-cne[string]$Receipt.preparation_id-or[string]$Context.compatibility.protocol_id-cne[string]$pointer.protocol_id){throw 'Preparation tooling context has a detached project, preparation, or protocol identity.'}
    if([string]$Source.schema-cne'rusty.morphospace.workflow.development_envelope_source_composition.v3'-or[string]$Source.tooling_protocol.protocol_id-cne[string]$pointer.protocol_id){throw 'Preparation tooling context requires a matching source-composition v3 protocol discriminator.'}
    $sourceProjection=$Context.product_projection.source_composition;$mapProjection=$Context.product_projection.repository_map;$lockProjection=$Context.product_projection.feature_lock
    if([string]$sourceProjection.path-cne[string]$Receipt.source_composition.path-or[string]$sourceProjection.sha256-cne(Get-PreparationByteHash $SourceBytes)-or[string]$mapProjection.path-cne[string]$Intent.target.repository_map.path-or[string]$mapProjection.sha256-cne[string]$Intent.target.repository_map.sha256-or[string]$lockProjection.path-cne'feature.lock.json'-or[string]$lockProjection.sha256-cne(Get-PreparationByteHash (Get-PreparationProtocolBytes $Intent.target.feature_lock.document))){throw 'Preparation tooling context changes or detaches an exact product projection.'}
    $liveSourcePath=Get-PreparationPath $Workspace ([string]$sourceProjection.path);if([IO.File]::Exists($liveSourcePath)-and(Get-PreparationFileHash $liveSourcePath)-cne[string]$sourceProjection.sha256){throw 'Preparation installed source-composition raw bytes differ from the tooling-context projection.'}
    $liveContextPath=Get-PreparationPath $Workspace ([string]$pointer.path);if([IO.File]::Exists($liveContextPath)-and(Get-PreparationFileHash $liveContextPath)-cne[string]$pointer.sha256){throw 'Preparation installed tooling-context raw bytes differ from its pointer.'}
    $resolver=Read-MorphospaceToolingContextResolver -WorkspaceRoot $Workspace -Context $Context
    if([IO.Path]::GetFullPath([string]$resolver.executor_root)-ine[IO.Path]::GetFullPath($RepoRoot)-or@($Context.executor.closure|Where-Object{([string]$_.path).Replace('\','/')-ceq'scripts/DevelopmentEnvelopePreparation.psm1'}).Count-ne1){throw 'Preparation is not executing from the context-bound executor and closed preparation module.'}
    ...
    $hasReclassification=Test-PreparationHasToolingReclassification $Receipt;if($hasReclassification-ne(Test-PreparationHasToolingReclassification $Intent)){throw 'Preparation receipt and intent disagree about legacy tooling reclassification.'}
    if($hasReclassification){
        if((Get-PreparationHash $Receipt.legacy_tooling_reclassification)-cne(Get-PreparationHash $Intent.legacy_tooling_reclassification)){throw 'Preparation legacy tooling reclassification binding differs between receipt and intent.'}
        $history=Get-PreparationLegacyToolingHistory -Workspace $Workspace -PreparationEventId ([string]$Intent.event.event_id)
        $oldMap=Read-MorphospaceProtocolJson (Get-PreparationPath $Workspace ([string]$Intent.pre.repository_map.path))
        $targetMap=Read-MorphospaceProtocolJson (Get-PreparationPath $Workspace ([string]$Intent.target.repository_map.path))
        [void](Assert-MorphospaceLegacyToolingReclassification -WorkspaceRoot $Workspace -Reclassification $Receipt.legacy_tooling_reclassification -CurrentProject $Intent.pre.project.document -TargetProject $Intent.target.project.document -CurrentMap $oldMap -TargetMap $targetMap -ToolingDescriptor $Context -History $history -TargetSourceRepositoryIds @($Source.repositories|ForEach-Object{[string]$_.repo_id}) -OwnerRepositories @($Receipt.envelope.owner_repositories))
    }
}
function Complete-MorphospaceDevelopmentEnvelopePreparation {
    param([string]$Workspace,[string]$RepoRoot,[string]$IntentRelative,[string]$CompletionRelative,[Management.Automation.PSModuleInfo]$OwnerModule,[switch]$CheckOnly,[ValidateSet('none','after-artifacts','after-project','after-lock','after-state','after-event')][string]$FaultAfter='none')
    ...
    # An interrupted ordinary preparation is not a looser recovery route: prove
    # the authored receipt/envelope and complete target semantics before any
    # staged artifact, projection, or event can be written.
    $receiptArtifact=@($intent.artifacts|Where-Object{[string]$_.path-ceq"receipts/$(([string]$intent.transaction_id)-replace'-prepared-transition$','').json"})
    if($receiptArtifact.Count-ne1){throw 'Preparation intent lacks one exact receipt artifact.'}
    $receipt=([Text.UTF8Encoding]::new($false).GetString([Convert]::FromBase64String([string]$receiptArtifact[0].bytes_base64))|ConvertFrom-Json -DateKind String)
    if((Get-PreparationHash $receipt.envelope.project)-cne[string]$intent.target.project.sha256-or(Get-PreparationHash $receipt.envelope.feature_lock)-cne[string]$intent.target.feature_lock.sha256-or[string]$receipt.project_sha256-cne[string]$intent.target.project.sha256-or[string]$receipt.feature_lock_sha256-cne[string]$intent.target.feature_lock.sha256){throw 'Preparation resume receipt envelope is not exactly bound to intent targets.'}
    if([string]$receipt.source_composition.path-cne[string]$receipt.envelope.source_composition.path){throw 'Preparation resume receipt source-composition path is not exactly bound to its envelope.'}
    foreach($side in @('pre','target')){foreach($name in @('project','state','feature_lock','predecessor_unit')){$binding=$intent.$side.$name;if((Get-PreparationHash $binding.document)-cne[string]$binding.sha256){throw "Preparation resume $side $name document is not authenticated by its binding hash."}}}
    [void](Get-MorphospaceDevelopmentEnvelopeCurrentProjectProjection $intent.pre.project.document)
    if(-not(Test-Json -Json ($intent.target.project.document|ConvertTo-Json -Depth 64) -SchemaFile (Join-Path $RepoRoot 'schemas\project-spec-v2.schema.json'))){throw 'Preparation resume target project projection is invalid.'}
    foreach($binding in @($intent.pre.feature_lock,$intent.target.feature_lock)) { if(-not(Test-Json -Json ($binding.document|ConvertTo-Json -Depth 64) -SchemaFile (Join-Path $RepoRoot 'schemas\feature-lock-v2.schema.json'))){throw 'Preparation resume feature-lock projection is invalid.'} }
    if((Get-PreparationHash $intent.target.state.document)-cne[string]$intent.target.state.sha256){throw 'Preparation resume target state document is not authenticated by its target hash.'}
    if(-not(Test-Json -Json ($intent.target.state.document|ConvertTo-Json -Depth 64) -SchemaFile (Join-Path $RepoRoot 'schemas\workspace-state-v2.schema.json'))){throw 'Preparation resume target workspace state projection is invalid.'}
    $resumePreparation=[pscustomobject]@{project_id=[string]$receipt.project_id;envelope=$receipt.envelope}
    $replayMode=$(if(Test-PreparationHasToolingReclassification $receipt){'ordinary'}else{Get-MorphospaceDevelopmentEnvelopeReplayMode $intent.pre.project.document $intent.target.project.document})
    $replayCurrent=$intent.pre.project.document
    if(Test-PreparationHasToolingReclassification $receipt){$removed=@($receipt.legacy_tooling_reclassification.removed_tool_repositories|ForEach-Object{[string]$_.repo_id});$replayCurrent=Copy-PreparationValue $intent.pre.project.document;$replayCurrent.repositories=@($replayCurrent.repositories|Where-Object{$removed-cnotcontains[string]$_.repo_id})}
    Assert-MorphospaceDevelopmentEnvelopeAdditiveProject $replayCurrent $intent.target.project.document ($null-ne$receipt.envelope.psobject.Properties['schema_pin_revision']) @($receipt.envelope.owner_repositories) $replayMode
    Assert-MorphospaceDevelopmentEnvelope $resumePreparation $intent.pre.project.document $intent.pre.feature_lock.document $replayMode
    $derivedState=Get-MorphospaceDevelopmentEnvelopeTargetState $resumePreparation $intent.pre.project.document $intent.pre.feature_lock.document $intent.pre.state.document
    $derivedState.last_event_id=[string]$intent.event.event_id;if((Get-PreparationHash $derivedState)-cne[string]$intent.target.state.sha256){throw 'Preparation resume target state is not the exact derived projection.'}
    ...
    if((Get-PreparationFileHash (Get-PreparationPath $Workspace ([string]$intent.target.repository_map.path)))-cne[string]$intent.target.repository_map.sha256){throw 'Preparation recovery target repository map is stale.'}
    $sourcePath=[string]$receipt.source_composition.path;$hasTooling=Test-PreparationHasToolingContext $receipt;if($hasTooling-ne(Test-PreparationHasToolingContext $intent)){throw 'Preparation receipt and intent disagree about tooling-context presence.'};$contextPath=$(if($hasTooling){[string]$receipt.tooling_context.path}else{''});$sourceArtifact=@($intent.artifacts|Where-Object{[string]$_.path-ceq$sourcePath});$contextArtifact=@($intent.artifacts|Where-Object{[string]$_.path-ceq$contextPath});$expectedArtifactCount=$(if($hasTooling){3}else{2});if($sourceArtifact.Count-ne1-or@($intent.artifacts).Count-ne$expectedArtifactCount-or($hasTooling-and$contextArtifact.Count-ne1)){throw 'Preparation intent lacks its exact staged source-composition and optional tooling-context artifacts.'}
    $validatedArtifacts=@();foreach($artifact in @($intent.artifacts)){$bytes=[Convert]::FromBase64String([string]$artifact.bytes_base64);$document=$(if($hasTooling){ConvertFrom-MorphospaceProtocolJsonBytes -Bytes $bytes -Context "preparation staged artifact '$($artifact.path)'"}else{[Text.UTF8Encoding]::new($false).GetString($bytes)|ConvertFrom-Json -DateKind String});if((Get-PreparationHash $document)-cne[string]$artifact.sha256){throw "Preparation staged artifact '$($artifact.path)' bytes do not authenticate its hash."};$schema=if([string]$artifact.path-ceq$sourcePath){$(if($hasTooling){'development-envelope-source-composition-v3.schema.json'}else{'development-envelope-source-composition-v1.schema.json'})}elseif($hasTooling-and[string]$artifact.path-ceq$contextPath){'tooling-context-v1.schema.json'}else{'development-envelope-preparation-receipt-v1.schema.json'};if(-not(Test-Json -Json ($document|ConvertTo-Json -Depth 64) -SchemaFile (Join-Path $RepoRoot ('schemas\\'+$schema)))){throw "Preparation staged artifact '$($artifact.path)' does not satisfy its schema."};if([string]$artifact.path-ceq$sourcePath-and[string]$receipt.source_composition.sha256-ne[string]$artifact.sha256){throw 'Preparation staged source-composition artifact does not match its receipt binding.'};$validatedArtifacts+=,[pscustomobject]@{artifact=$artifact;bytes=$bytes;document=$document}}
    if($hasTooling){$sourceValidated=@($validatedArtifacts|Where-Object{[string]$_.artifact.path-ceq$sourcePath});$contextValidated=@($validatedArtifacts|Where-Object{[string]$_.artifact.path-ceq$contextPath});Assert-PreparationToolingContextArtifact $Workspace $RepoRoot $receipt $intent $sourceValidated[0].document $sourceValidated[0].bytes $contextValidated[0].document $contextValidated[0].bytes $OwnerModule}
    ...
    foreach ($validated in $validatedArtifacts) {
        $artifact=$validated.artifact
        $path=Get-PreparationPath $Workspace ([string]$artifact.path)
        if ([IO.File]::Exists($path) -and (Get-PreparationHash (Read-MorphospaceProtocolJson $path)) -cne [string]$artifact.sha256) { throw "Preparation artifact '$($artifact.path)' conflicts with intent." }
    }
    $eventsPath=Get-PreparationPath $Workspace ([string]$intent.pre.events.path)
    $events=@(Get-Content -LiteralPath $eventsPath | Where-Object { $_ } | ForEach-Object { $_ | ConvertFrom-Json -DateKind String })
    $same=@($events | Where-Object { [string]$_.event_id -ceq [string]$intent.event.event_id })
    if ($same.Count -gt 1 -or ($same.Count -eq 1 -and ((Get-PreparationHash $same[0]) -cne (Get-PreparationHash $intent.event) -or [string]$events[-1].event_id -cne [string]$intent.event.event_id))) { throw 'Preparation event placement conflicts with intent.' }
    if ($same.Count -eq 0) {
        if ((Get-PreparationFileHash $eventsPath) -cne [string]$intent.pre.events.sha256 -or [int]$events[-1].sequence+1 -ne [int]$intent.event.sequence) { throw 'Preparation event predecessor is stale.' }
    } else {
        $bytes=[IO.File]::ReadAllBytes($eventsPath)
        $offset=0;$lineCount=0
        ...
        if ($lineCount -ne [int]$intent.event.sequence-1 -or (Get-MorphospaceSha256Bytes $prefix) -cne [string]$intent.pre.events.sha256) { throw 'Preparation event predecessor is stale.' }
    }
    foreach($validated in $validatedArtifacts){$artifact=$validated.artifact;$path=Get-PreparationPath $Workspace ([string]$artifact.path);if([IO.File]::Exists($path)){if((Get-PreparationHash (Read-MorphospaceProtocolJson $path))-cne[string]$artifact.sha256){throw "Preparation artifact '$($artifact.path)' conflicts with intent."}}elseif(-not$CheckOnly){[IO.Directory]::CreateDirectory([IO.Path]::GetDirectoryName($path))|Out-Null;[IO.File]::WriteAllBytes($path,$validated.bytes)}}
    ...
    if (-not $CheckOnly -and $same.Count -eq 0) { [IO.File]::AppendAllText($eventsPath,(($intent.event | ConvertTo-Json -Compress)+"`n"),[Text.UTF8Encoding]::new($false)) }
...
function Get-PreparationSourceComposition {
    ...
    $identity=[pscustomobject][ordered]@{project_id=$Preparation.project_id;preparation_id=$Preparation.preparation_id;repositories=@($records.ToArray())}
    if(Test-PreparationHasToolingContext $Preparation){$identity|Add-Member -NotePropertyName tooling_protocol -NotePropertyValue ([pscustomobject][ordered]@{protocol_id=[string]$Preparation.tooling_context.compatibility.protocol_id})}
    $fingerprint=Get-PreparationHash $identity
    $source=[pscustomobject][ordered]@{schema=$(if(Test-PreparationHasToolingContext $Preparation){'rusty.morphospace.workflow.development_envelope_source_composition.v3'}else{'rusty.morphospace.workflow.development_envelope_source_composition.v1'});lock_id="$($Preparation.preparation_id)-source-$($fingerprint.Substring(0,12))";preparation_id=$Preparation.preparation_id;project_id=$Preparation.project_id;fingerprint=$fingerprint;repositories=@($records.ToArray())}
    if(Test-PreparationHasToolingContext $Preparation){$source|Add-Member -NotePropertyName tooling_protocol -NotePropertyValue $identity.tooling_protocol}
    $source|Add-Member -NotePropertyName status -NotePropertyValue 'locked'
    [object[]]$doesNotProve=if(Test-PreparationHasToolingContext $Preparation){@('Does not admit a future unit, claim a device, enable a feature, execute a build, authorize publication, or place tooling in product source authority.')}else{@('Does not admit a future unit, claim a device, enable a feature, execute a build, or authorize publication.')}
    $source|Add-Member -NotePropertyName does_not_prove -NotePropertyValue $doesNotProve
    $source
}
function Invoke-MorphospacePrepareDevelopmentEnvelope {
 ...
 $ownerModule=$MyInvocation.MyCommand.Module;$repoRoot=Split-Path $PSScriptRoot -Parent;$workspace=(Resolve-Path $WorkspaceRoot).Path;$input=(Resolve-Path $DevelopmentEnvelopePreparation).Path
 Assert-PreparationSchema $repoRoot $input 'development-envelope-preparation-v1.schema.json' 'Development envelope preparation does not satisfy its schema.';$p=Read-MorphospaceProtocolJson $input;$inputHash=Get-PreparationFileHash $input;$isReclassification=$p.PSObject.Properties.Name-ccontains'preparation_kind'-and[string]$p.preparation_kind-ceq'legacy-tooling-reclassification'
 ...
 $earlyIntent=Get-PreparationPath $workspace $intentRelative
  if([IO.File]::Exists($earlyIntent)){$intent=Read-MorphospaceProtocolJson $earlyIntent;$receiptArtifact=@($intent.artifacts|Where-Object{[string]$_.path-ceq$receiptRelative});if($receiptArtifact.Count-ne1){throw 'Preparation replay intent lacks its exact receipt artifact.'};$intentReceipt=([Text.UTF8Encoding]::new($false).GetString([Convert]::FromBase64String([string]$receiptArtifact[0].bytes_base64))|ConvertFrom-Json);if([string]$intent.transaction_id-cne$transactionId-or[string]$intentReceipt.input_sha256-cne$inputHash-or(Get-PreparationHash $intent.target.project.document)-cne(Get-PreparationHash $p.envelope.project)-or(Get-PreparationHash $intent.target.feature_lock.document)-cne(Get-PreparationHash $p.envelope.feature_lock)-or(Get-PreparationHash $intent.target.state.document)-cne[string]$intent.target.state.sha256){throw 'Preparation replay conflicts with its published intent.'};[void](Complete-MorphospaceDevelopmentEnvelopePreparation $workspace $repoRoot $intentRelative $completionRelative -OwnerModule $ownerModule -CheckOnly);if([IO.File]::Exists((Get-PreparationPath $workspace $completionRelative))){Assert-PreparationHistoricalSupersessionClosure $workspace};$liveState=Read-MorphospaceProtocolJson (Get-PreparationPath $workspace 'workspace.state.json');if($Execute){$mutex=Enter-MorphospaceWorkspaceMutex $workspace;try{[void](Complete-MorphospaceDevelopmentEnvelopePreparation $workspace $repoRoot $intentRelative $completionRelative -OwnerModule $ownerModule -FaultAfter $FaultAfter)}finally{Exit-MorphospaceWorkspaceMutex $mutex}};return (New-PreparationAutomationReceipt $p $Timestamp $Execute.IsPresent 'idle-project-envelope-prepared' $receiptRelative $inputHash $liveState $(if($Execute){$eventId}else{$null}))}
 $projectPath=Get-PreparationPath $workspace 'project.spec.json';$statePath=Get-PreparationPath $workspace 'workspace.state.json';$lockPath=Get-PreparationPath $workspace 'feature.lock.json';$eventsPath=Get-PreparationPath $workspace 'iteration-events.jsonl';$mapPath=Get-PreparationPath $workspace ([string]$p.expected.repository_map_path);$targetMapRelative=$(if($isReclassification){[string]$p.legacy_tooling_reclassification.target_repository_map.path}else{[string]$p.expected.repository_map_path});$targetMapPath=Get-PreparationPath $workspace $targetMapRelative;$prePath=Get-PreparationPath $workspace ([string]$p.expected.predecessor_unit_path)
 $project=Read-MorphospaceProtocolJson $projectPath;$state=Read-MorphospaceProtocolJson $statePath;$lock=Read-MorphospaceProtocolJson $lockPath;$mapDoc=Read-MorphospaceProtocolJson $mapPath;$targetMapDoc=$(if($isReclassification){Read-MorphospaceProtocolJson $targetMapPath}else{$mapDoc});$pre=Read-MorphospaceProtocolJson $prePath;$eventBytes=[IO.File]::ReadAllBytes($eventsPath);$events=@(Get-Content $eventsPath|Where-Object{$_}|ForEach-Object{$_|ConvertFrom-Json});if($events.Count-eq0){throw 'Preparation requires a predecessor event.'};$tail=$events[-1]
 [void](Get-MorphospaceDevelopmentEnvelopeCurrentProjectProjection $project);Assert-PreparationSchema $repoRoot $lockPath 'feature-lock-v2.schema.json' 'Preparation current feature lock does not satisfy the owner schema.'
 ...
 if($isReclassification){Assert-PreparationSchema $repoRoot $targetMapPath 'repository-map.schema.json' 'Preparation target repository map does not satisfy the closed owner schema.'}
 ...
 if([int64]$p.expected.events_length-ne$eventBytes.LongLength-or[string]$p.expected.event_tail_id-cne[string]$tail.event_id){throw 'Preparation ledger predecessor is stale.'};$map=@{};foreach($entry in @($mapDoc.repositories)){$id=[string]$entry.repo_id;if($map.ContainsKey($id)){throw "Preparation repository map repeats '$id' case-insensitively."};$map[$id]=$entry};$targetMap=@{};foreach($entry in @($targetMapDoc.repositories)){$id=[string]$entry.repo_id;if($targetMap.ContainsKey($id)){throw "Preparation target repository map repeats '$id' case-insensitively."};$targetMap[$id]=$entry}
 ...
 if($isReclassification){
  if(-not(Test-PreparationHasToolingContext $p)){throw 'Legacy tooling reclassification requires a new tooling context.'};if($null-eq(Get-Command Get-MorphospaceCurrentWorkHistory -ErrorAction SilentlyContinue)){Import-Module (Join-Path $PSScriptRoot 'lib/MorphospaceCurrentWorkHistory.psm1')};$history=Get-MorphospaceCurrentWorkHistory -WorkspaceRoot $workspace -RequireIdle
  [void](Assert-MorphospaceLegacyToolingReclassification -WorkspaceRoot $workspace -Reclassification $p.legacy_tooling_reclassification -CurrentProject $project -TargetProject $p.envelope.project -CurrentMap $mapDoc -TargetMap $targetMapDoc -ToolingDescriptor $p.tooling_context -History $history -TargetSourceRepositoryIds @($p.envelope.source_composition.repository_ids) -OwnerRepositories @($p.envelope.owner_repositories))
  $removed=@($p.legacy_tooling_reclassification.removed_tool_repositories|ForEach-Object{[string]$_.repo_id});$filteredProject=Copy-PreparationValue $project;$filteredProject.repositories=@($filteredProject.repositories|Where-Object{$removed-cnotcontains[string]$_.repo_id});Assert-PreparationAdditiveProject $filteredProject $p.envelope.project ($null-ne$p.envelope.psobject.Properties['schema_pin_revision']) @($p.envelope.owner_repositories) 'ordinary'
 }else{Assert-PreparationAdditiveProject $project $p.envelope.project ($null-ne$p.envelope.psobject.Properties['schema_pin_revision']) @($p.envelope.owner_repositories) 'ordinary'}
 Assert-PreparationRoots @($p.envelope.owner_repositories) $p.envelope.project $targetMap;Assert-PreparationEnvelope $p $project $lock ordinary;Assert-PreparationLockAndRegistry $p.envelope.project $p.envelope.feature_lock $targetState 'target'
 $ownerIds=@($p.envelope.owner_repositories|ForEach-Object{[string]$_.repo_id}|Sort-Object -Unique);$sourceIds=@($p.envelope.source_composition.repository_ids|Sort-Object -Unique);$mapIds=@($targetMap.Keys|Sort-Object -Unique);if($ownerIds.Count-ne$sourceIds.Count-or$ownerIds.Count-ne$mapIds.Count-or(@($ownerIds|Where-Object{$sourceIds-cnotcontains$_-or$mapIds-cnotcontains$_}).Count-ne0)){throw 'Preparation owner repositories, target repository map, and source-lock repository sets must agree exactly.'}
 ...
 $hasTooling=Test-PreparationHasToolingContext $p;$source=Get-PreparationSourceComposition $p $targetMap;$sourceSchema=$(if($hasTooling){'development-envelope-source-composition-v3.schema.json'}else{'development-envelope-source-composition-v1.schema.json'});if(-not(Test-Json -Json ($source|ConvertTo-Json -Depth 32) -SchemaFile (Join-Path $repoRoot "schemas\$sourceSchema"))){throw 'Preparation generated source lock does not satisfy its closed schema.'};Assert-PreparationSchema $repoRoot ([string]$input) 'development-envelope-preparation-v1.schema.json' 'Preparation input changed during observation.'
 $targetState.last_event_id=$eventId;$toolingContext=$null;$toolingPointer=$null;$sourceBytes=$(if($hasTooling){Get-PreparationProtocolBytes $source}else{[Text.UTF8Encoding]::new($false).GetBytes(($source|ConvertTo-Json -Depth 64))})
 if($hasTooling){
  $descriptor=$p.tooling_context;$contextRelative=[string]$descriptor.path
  if($contextRelative-ceq$sourceRelative-or$contextRelative-ceq$receiptRelative-or$contextRelative.StartsWith('local/',[StringComparison]::Ordinal)){throw 'Preparation tooling-context artifact path must be portable and distinct from source, receipt, and ignored resolver paths.'}
  $resolverPath=Get-PreparationPath $workspace ([string]$descriptor.resolver.path);if((Get-PreparationFileHash $resolverPath)-cne[string]$descriptor.resolver.sha256){throw 'Preparation tooling resolver bytes drifted before observation.'}
  $productProjection=[pscustomobject][ordered]@{source_composition=[pscustomobject][ordered]@{path=$sourceRelative;sha256=(Get-PreparationByteHash $sourceBytes)};repository_map=[pscustomobject][ordered]@{path=$targetMapRelative;sha256=(Get-PreparationFileHash $targetMapPath)};feature_lock=[pscustomobject][ordered]@{path='feature.lock.json';sha256=(Get-PreparationByteHash (Get-PreparationProtocolBytes $p.envelope.feature_lock))}}
  $toolingContext=New-MorphospaceObservedToolingContext -WorkspaceRoot $workspace -ContextId ([string]$descriptor.context_id) -ProjectId ([string]$p.project_id) -PreparationId ([string]$p.preparation_id) -ProductProjection $productProjection -ResolverPath ([string]$descriptor.resolver.path) -Executor $descriptor.executor -Routers @($descriptor.routers) -Compatibility $descriptor.compatibility
  if([string]$toolingContext.resolver.path-cne[string]$descriptor.resolver.path-or[string]$toolingContext.resolver.sha256-cne[string]$descriptor.resolver.sha256){throw 'Preparation observed tooling context does not retain its exact authored resolver binding.'}
  $toolingBytes=Get-PreparationProtocolBytes $toolingContext;$toolingPointer=[pscustomobject][ordered]@{path=$contextRelative;sha256=(Get-PreparationByteHash $toolingBytes);canonical_sha256=(Get-PreparationHash $toolingContext);protocol_id=[string]$descriptor.compatibility.protocol_id}
  $resolver=Read-MorphospaceToolingContextResolver -WorkspaceRoot $workspace -Context $toolingContext;if([IO.Path]::GetFullPath([string]$resolver.executor_root)-ine[IO.Path]::GetFullPath($repoRoot)-or@($toolingContext.executor.closure|Where-Object{([string]$_.path).Replace('\','/')-ceq'scripts/DevelopmentEnvelopePreparation.psm1'}).Count-ne1){throw 'Preparation is not executing from the context-bound executor and closed preparation module.'}
  [void](Assert-MorphospaceToolingContextLocalObservation -Context $toolingContext -WorkspaceRoot $workspace)
  [void](Assert-MorphospaceToolingContextLoadedOwnerModule -Context $toolingContext -ExecutorRoot $repoRoot -OwnerModule $ownerModule)
 }
 if($hasTooling){$receipt=[pscustomobject][ordered]@{schema='rusty.morphospace.workflow.development_envelope_preparation_receipt.v1';preparation_id=$p.preparation_id;project_id=$p.project_id;predecessor_unit_id=$p.predecessor_unit_id;input_sha256=$inputHash;envelope=$p.envelope;project_sha256=(Get-PreparationHash $p.envelope.project);feature_lock_sha256=(Get-PreparationHash $p.envelope.feature_lock);source_composition=[pscustomobject]@{path=$sourceRelative;sha256=(Get-PreparationHash $source)};tooling_context=$toolingPointer;does_not_prove=@('Does not admit, Ready, Inspect, Claim, amend, freeze, validate, accept, approve a schema revision, build, deploy, launch, install tooling, mutate a device, mutate a Git remote, or publish a future unit.')}}else{$receipt=[pscustomobject][ordered]@{schema='rusty.morphospace.workflow.development_envelope_preparation_receipt.v1';preparation_id=$p.preparation_id;project_id=$p.project_id;predecessor_unit_id=$p.predecessor_unit_id;input_sha256=$inputHash;envelope=$p.envelope;project_sha256=(Get-PreparationHash $p.envelope.project);feature_lock_sha256=(Get-PreparationHash $p.envelope.feature_lock);source_composition=[pscustomobject]@{path=$sourceRelative;sha256=(Get-PreparationHash $source)};does_not_prove=@('Does not admit, Ready, Inspect, Claim, amend, freeze, validate, accept, approve a schema revision, build, deploy, launch, mutate a device, mutate a Git remote, or publish a future unit.')}}
 if($isReclassification){$receipt|Add-Member -NotePropertyName legacy_tooling_reclassification -NotePropertyValue (Copy-PreparationValue $p.legacy_tooling_reclassification)}
 ...
 $preBinding=[pscustomobject]@{project=[pscustomobject]@{path='project.spec.json';sha256=(Get-PreparationHash $project);document=$project};state=[pscustomobject]@{path='workspace.state.json';sha256=(Get-PreparationHash $state);document=$state};feature_lock=[pscustomobject]@{path='feature.lock.json';sha256=(Get-PreparationHash $lock);document=$lock};predecessor_unit=[pscustomobject]@{path=$p.expected.predecessor_unit_path;sha256=(Get-PreparationHash $pre);document=$pre};events=[pscustomobject]@{path='iteration-events.jsonl';sha256=(Get-PreparationFileHash $eventsPath)};repository_map=[pscustomobject]@{path=$p.expected.repository_map_path;sha256=(Get-PreparationFileHash $mapPath)}};$targetBinding=[pscustomobject]@{project=[pscustomobject]@{path='project.spec.json';sha256=(Get-PreparationHash $p.envelope.project);document=$p.envelope.project};state=[pscustomobject]@{path='workspace.state.json';sha256=(Get-PreparationHash $targetState);document=$targetState};feature_lock=[pscustomobject]@{path='feature.lock.json';sha256=(Get-PreparationHash $p.envelope.feature_lock);document=$p.envelope.feature_lock};predecessor_unit=[pscustomobject]@{path=$p.expected.predecessor_unit_path;sha256=(Get-PreparationHash $pre);document=$pre};events=[pscustomobject]@{path='iteration-events.jsonl';sha256=(Get-PreparationFileHash $eventsPath)};repository_map=[pscustomobject]@{path=$targetMapRelative;sha256=(Get-PreparationFileHash $targetMapPath)}}
 if($hasTooling){$receiptBytes=Get-PreparationProtocolBytes $receipt;$artifacts=@([pscustomobject]@{path=$receiptRelative;sha256=(Get-PreparationHash $receipt);bytes_base64=[Convert]::ToBase64String($receiptBytes)},[pscustomobject]@{path=$sourceRelative;sha256=(Get-PreparationHash $source);bytes_base64=[Convert]::ToBase64String($sourceBytes)},[pscustomobject]@{path=[string]$toolingPointer.path;sha256=[string]$toolingPointer.canonical_sha256;bytes_base64=[Convert]::ToBase64String($toolingBytes)});$intent=[pscustomobject][ordered]@{schema='rusty.morphospace.workflow.development_envelope_preparation_intent.v1';transaction_id=$transactionId;created_at=$Timestamp;pre=$preBinding;target=$targetBinding;tooling_context=$toolingPointer;artifacts=$artifacts;event=$event;status='prepared'}}else{$intent=[pscustomobject][ordered]@{schema='rusty.morphospace.workflow.development_envelope_preparation_intent.v1';transaction_id=$transactionId;created_at=$Timestamp;pre=$preBinding;target=$targetBinding;artifacts=@([pscustomobject]@{path=$receiptRelative;sha256=(Get-PreparationHash $receipt);bytes_base64=[Convert]::ToBase64String([Text.UTF8Encoding]::new($false).GetBytes(($receipt|ConvertTo-Json -Depth 64)))},[pscustomobject]@{path=$sourceRelative;sha256=(Get-PreparationHash $source);bytes_base64=[Convert]::ToBase64String([Text.UTF8Encoding]::new($false).GetBytes(($source|ConvertTo-Json -Depth 64))) });event=$event;status='prepared'}}
 if($isReclassification){$intent|Add-Member -NotePropertyName legacy_tooling_reclassification -NotePropertyValue (Copy-PreparationValue $p.legacy_tooling_reclassification)}
