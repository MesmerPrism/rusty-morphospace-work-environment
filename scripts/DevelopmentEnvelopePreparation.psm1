...
function Complete-MorphospaceDevelopmentEnvelopePreparation {
    ...
    $replayMode=Get-MorphospaceDevelopmentEnvelopeReplayMode $intent.pre.project.document $intent.target.project.document
    Assert-MorphospaceDevelopmentEnvelopeAdditiveProject $intent.pre.project.document $intent.target.project.document ($null-ne$receipt.envelope.psobject.Properties['schema_pin_revision']) @($receipt.envelope.owner_repositories) $replayMode
    ...
    $sourcePath=[string]$receipt.source_composition.path;$sourceArtifact=@($intent.artifacts|Where-Object{[string]$_.path-ceq$sourcePath});if($sourceArtifact.Count-ne1-or@($intent.artifacts).Count-ne2){throw 'Preparation intent lacks one exact staged source-composition artifact.'}
    $validatedArtifacts=@();foreach($artifact in @($intent.artifacts)){$bytes=[Convert]::FromBase64String([string]$artifact.bytes_base64);$document=([Text.UTF8Encoding]::new($false).GetString($bytes)|ConvertFrom-Json -DateKind String);if((Get-PreparationHash $document)-cne[string]$artifact.sha256){throw "Preparation staged artifact '$($artifact.path)' bytes do not authenticate its hash."};$schema=if([string]$artifact.path-ceq$sourcePath){'development-envelope-source-composition-v1.schema.json'}else{'development-envelope-preparation-receipt-v1.schema.json'};if(-not(Test-Json -Json ($document|ConvertTo-Json -Depth 64) -SchemaFile (Join-Path $RepoRoot ('schemas\\'+$schema)))){throw "Preparation staged artifact '$($artifact.path)' does not satisfy its schema."};if([string]$artifact.path-ceq$sourcePath-and[string]$receipt.source_composition.sha256-ne[string]$artifact.sha256){throw 'Preparation staged source-composition artifact does not match its receipt binding.'};$validatedArtifacts+=,[pscustomobject]@{artifact=$artifact;bytes=$bytes}}
...
function Invoke-MorphospacePrepareDevelopmentEnvelope {
 ...
 Assert-PreparationAdditiveProject $project $p.envelope.project ($null-ne$p.envelope.psobject.Properties['schema_pin_revision']) @($p.envelope.owner_repositories) 'ordinary';Assert-PreparationRoots @($p.envelope.owner_repositories) $p.envelope.project $map;Assert-PreparationEnvelope $p $project $lock ordinary;Assert-PreparationLockAndRegistry $p.envelope.project $p.envelope.feature_lock $targetState 'target'
