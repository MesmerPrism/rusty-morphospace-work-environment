...
public static class W017BoundedChildCapture {
    ...
    private static bool IsTransientThreadOpenError(int error){return error==5||error==87;}
    public static void RunTransientThreadOpenErrorSelfTests(){if(!IsTransientThreadOpenError(5)||!IsTransientThreadOpenError(87)||IsTransientThreadOpenError(0)||IsTransientThreadOpenError(6)||IsTransientThreadOpenError(123))throw new InvalidOperationException("thread-open transient error classification self-test failed");}
...
public static class W017SupervisorProtection {
    ...
    private static void ProtectThreads(int processId,IntPtr descriptor,AncestorGuard guard){var snapshot=CreateToolhelp32Snapshot(4,0);if(snapshot==new IntPtr(-1))throw new InvalidOperationException(""trusted authority thread snapshot failed: ""+Marshal.GetLastWin32Error());var count=0;try{var entry=new ThreadEntry32();entry.size=(uint)Marshal.SizeOf(typeof(ThreadEntry32));if(Thread32First(snapshot,ref entry)){do{if(entry.ownerProcessId==unchecked((uint)processId)){var handle=OpenThread(0x000e0000,false,entry.threadId);if(handle==IntPtr.Zero){var error=Marshal.GetLastWin32Error();if(IsTransientThreadOpenError(error)){entry.size=(uint)Marshal.SizeOf(typeof(ThreadEntry32));continue;}throw new InvalidOperationException(""trusted authority thread could not be opened for protection: tid=""+entry.threadId+"" error=""+error);}try{var original=ReadSecurity(handle);if(!SetKernelObjectSecurity(handle,OwnerSecurityInformation|DaclSecurityInformation,descriptor))throw new InvalidOperationException(""trusted authority thread protection could not be installed: tid=""+entry.threadId+"" error=""+Marshal.GetLastWin32Error());count++;if(guard!=null){guard.AddThread(handle,original,entry.threadId);handle=IntPtr.Zero;}}finally{if(handle!=IntPtr.Zero)CloseHandle(handle);}}entry.size=(uint)Marshal.SizeOf(typeof(ThreadEntry32));}while(Thread32Next(snapshot,ref entry));}}finally{CloseHandle(snapshot);}if(count==0)throw new InvalidOperationException(""trusted authority process had no threads at protection readback: pid=""+processId);}
    ...
    private static bool SameIds(uint[] left,uint[] right){if(left.Length!=right.Length)return false;for(var index=0;index<left.Length;index++)if(left[index]!=right[index])return false;return true;}
    private static void RestoreProtectedFutureThreads(int processId,byte[] protectedDacl,byte[] originalSecurity,System.Collections.Generic.HashSet<uint> retainedThreadIds){var restoredThreadIds=new System.Collections.Generic.HashSet<uint>();var restoredThreadHandles=new System.Collections.Generic.List<IntPtr>();try{for(var attempt=0;attempt<16;attempt++){var before=SnapshotThreadIds(processId);var restored=0;foreach(var id in before){if(retainedThreadIds.Contains(id)||restoredThreadIds.Contains(id))continue;var handle=OpenThread(0x000e0000,false,id);if(handle==IntPtr.Zero){var error=Marshal.GetLastWin32Error();if(IsTransientThreadOpenError(error))continue;throw new InvalidOperationException(""trusted future-thread cleanup could not open thread: tid=""+id+"" error=""+error);}try{var observed=ReadSecurity(handle);if(SameSecurity(DaclBytes(observed),protectedDacl)){var value=Marshal.AllocHGlobal(originalSecurity.Length);try{Marshal.Copy(originalSecurity,0,value,originalSecurity.Length);if(!SetKernelObjectSecurity(handle,OwnerSecurityInformation|DaclSecurityInformation,value))throw new InvalidOperationException(""trusted future-thread cleanup could not restore thread: tid=""+id+"" error=""+Marshal.GetLastWin32Error());var readback=ReadSecurity(handle);if(!SameSecurity(originalSecurity,readback))throw new InvalidOperationException(""trusted future-thread cleanup readback differs from the exact default-thread template: tid=""+id);if(!restoredThreadIds.Add(id))throw new InvalidOperationException(""trusted future-thread cleanup observed a duplicate restored identity: tid=""+id);restoredThreadHandles.Add(handle);handle=IntPtr.Zero;restored++;}finally{Marshal.FreeHGlobal(value);}}}finally{if(handle!=IntPtr.Zero)CloseHandle(handle);}}var after=SnapshotThreadIds(processId);if(restored==0&&SameIds(before,after))return;System.Threading.Thread.Sleep(10);}throw new InvalidOperationException(""trusted future-thread cleanup did not reach a stable residue-free thread set"");}finally{for(var index=restoredThreadHandles.Count-1;index>=0;index--)CloseHandle(restoredThreadHandles[index]);}}
    ...
    public static void RunRestorationCollisionSelfTests(){RunTransientThreadOpenErrorSelfTests();RunRestorationCollisionCase(false);RunRestorationCollisionCase(true);}
...
function New-AffectedValidationDependencyProjectionFile([object]$Check,[object[]]$DependencyManifest) {
    ...
    $projection = [pscustomobject][ordered]@{
        ...
        head_commit=[string]$plan.head.commit
        head_tree=[string]$plan.head.tree
    ...
    $json = ConvertTo-MorphospaceCanonicalJson -Value $projection
    $schemaPath = Join-Path $repoRoot 'schemas/affected-validation-self-test-dependency-projection-v1.schema.json'
    if (-not (Test-Json -Json $json -SchemaFile $schemaPath -ErrorAction Stop)) { throw 'Affected-validation dependency projection fails its closed schema before publication.' }
    [byte[]]$bytes = [Text.UTF8Encoding]::new($false,$true).GetBytes($json + "`n")
    if ($bytes.Length -gt 134217728) { throw 'Affected-validation dependency projection exceeds its publication bound.' }
    $path = Join-Path ([IO.Path]::GetTempPath()) ('.rusty-affected-dependency-projection-' + $PID + '-' + [Guid]::NewGuid().ToString('N') + '.json')
    ...
    try { $stream.Write($bytes,0,$bytes.Length);$stream.Flush($true) } catch {
        $publicationFailure = $_
        $publicationCleanupError = $null
        try { $stream.Dispose() } catch { $publicationCleanupError = "dependency projection handle cleanup failed: $($_.Exception.Message)" }
        try {
            if ([IO.File]::Exists($path)) { Remove-Item -LiteralPath $path -Force -ErrorAction Stop }
            if ([IO.File]::Exists($path)) { throw 'projection path remains present' }
        } catch {
            $removeFailure = "dependency projection file cleanup failed: $($_.Exception.Message)"
            $publicationCleanupError = if ($null -eq $publicationCleanupError) { $removeFailure } else { "$publicationCleanupError $removeFailure" }
        }
        if ($null -ne $publicationCleanupError) { throw "Affected-validation dependency projection publication failed and cleanup did not complete: $publicationCleanupError Original failure: $($publicationFailure.Exception.Message)" }
        throw $publicationFailure
    }
    return [pscustomobject][ordered]@{path=$path;sha256=Get-AffectedValidationBytesHash $bytes;bytes=$bytes;stream=$stream}
}
function Get-AffectedValidationDependencyClosureCacheKey([object]$Check) {
    return Get-MorphospaceCanonicalJsonSha256 -Value ([pscustomobject][ordered]@{
        head_commit=[string]$plan.head.commit
        head_tree=[string]$plan.head.tree
        registry_sha256=[string]$registrySha256
        command_path=[string]$Check.command_path
...
function Assert-AffectedValidationEnvironmentEvidence([object]$Evidence,[bool]$RequireProjection) {
    if([bool]$Evidence.projected-ne$RequireProjection){throw 'Affected-validation environment projection mode is inconsistent.'}
    if(-not[bool]$Evidence.parent_environment_unchanged){throw 'Affected-validation parent environment was not preserved.'}
    $runtime=@('COMSPEC','HOME','PATH','PATHEXT','SYSTEMROOT','TEMP','TMP','TMPDIR','WINDIR','RUNNER_TEMP','PSExecutionPolicyPreference','PSModulePath');$hosted=@('GITHUB_ACTIONS','GITHUB_REPOSITORY','GITHUB_EVENT_NAME','GITHUB_RUN_ID','GITHUB_RUN_ATTEMPT','GITHUB_WORKFLOW_REF','GITHUB_JOB','PR_NUMBER');$owned=@('RUSTY_AFFECTED_VALIDATION_PHASE_ROOT','RUSTY_AFFECTED_VALIDATION_BASE_COMMIT','RUSTY_AFFECTED_VALIDATION_HEAD_COMMIT','RUSTY_AFFECTED_VALIDATION_PLAN_SHA256','RUSTY_AFFECTED_VALIDATION_PLATFORM','RUSTY_AFFECTED_VALIDATION_CHECK_ID','RUSTY_AFFECTED_VALIDATION_DEPENDENCY_PROJECTION_PATH','RUSTY_AFFECTED_VALIDATION_DEPENDENCY_PROJECTION_SHA256','RUSTY_AFFECTED_VALIDATION_GUARD_SID','RUSTY_AFFECTED_VALIDATION_TRUSTED_ANCESTORS','RUSTY_AFFECTED_VALIDATION_PARENT_FUTURE_THREAD_ID','RUSTY_AFFECTED_VALIDATION_REMOVED_PRIVILEGE_LUID','RUSTY_AFFECTED_VALIDATION_FUTURE_THREAD_ID')
    foreach($hop in @('supervisor','leaf')){$variables=@($Evidence."${hop}_variables");if([int]$Evidence."${hop}_variable_count"-ne$variables.Count){throw "Affected-validation $hop environment count is inconsistent."};$previous=$null;foreach($record in $variables){$name=[string]$record.name;$source=[string]$record.source;if($null-ne$previous-and[StringComparer]::Ordinal.Compare($previous,$name)-ge0){throw "Affected-validation $hop environment is not unique and ordinal sorted."};$valid=($source-ceq'host-runtime'-and$runtime-ccontains$name)-or($source-ceq'hosted-producer'-and$hosted-ccontains$name)-or($source-ceq'launcher-owned'-and$owned-ccontains$name);if(-not$valid){throw "Affected-validation $hop environment contains an unreviewed variable: $name ($source)."};$previous=$name}}
    if($RequireProjection){if([string]$Evidence.parent_sha256_before-cne[string]$Evidence.parent_sha256_after-or[int]$Evidence.parent_variable_count_before-ne[int]$Evidence.parent_variable_count_after-or[string]$Evidence.supervisor_sha256-cne(Get-MorphospaceCanonicalJsonSha256 -Value @($Evidence.supervisor_variables))){throw 'Affected-validation projected environment hashes are incomplete or inconsistent.'};if(($variables=@($Evidence.leaf_variables)).Count-eq0){if($null-ne$Evidence.leaf_sha256-or[int]$Evidence.leaf_variable_count-ne0){throw 'Affected-validation pre-leaf projection evidence is not null/zero.'}}else{if([string]$Evidence.leaf_sha256-cne(Get-MorphospaceCanonicalJsonSha256 -Value @($Evidence.leaf_variables))){throw 'Affected-validation leaf environment hash is absent or inconsistent.'};$leafByName=@{};foreach($record in $variables){$leafByName[[string]$record.name]=$record};foreach($record in @($Evidence.supervisor_variables|Where-Object{$_.source-ceq'hosted-producer'-or$_.source-ceq'launcher-owned'})){$name=[string]$record.name;if(-not$leafByName.ContainsKey($name)-or[string]$leafByName[$name].source-cne[string]$record.source-or[string]$leafByName[$name].value_sha256-cne[string]$record.value_sha256){throw "Affected-validation second-hop environment changed exact projected variable '$name'."}}}}elseif([int]$Evidence.supervisor_variable_count-ne0-or$null-ne$Evidence.supervisor_sha256-or@($Evidence.supervisor_variables).Count-ne0-or[int]$Evidence.leaf_variable_count-ne0-or$null-ne$Evidence.leaf_sha256-or@($Evidence.leaf_variables).Count-ne0){throw 'Affected-validation preprojection environment evidence is not null/zero.'}
}
function Invoke-AffectedValidationCheck([object]$Check, [string]$Command, [string[]]$IntegrityPaths, [object]$Inventory, [object[]]$DependencyManifest) {
        ...
        if ($null -ne $dependencyProjectionFile) {
            try { if ($null -ne $dependencyProjectionFile.stream) { $dependencyProjectionFile.stream.Dispose(); $dependencyProjectionFile.stream = $null } } catch { $projectionCleanupError = "Affected-check dependency projection handle cleanup failed: $($_.Exception.Message)" }
                ...
                try {
                    ...
                    if ([IO.File]::Exists([string]$dependencyProjectionFile.path)) { throw 'projection path remains present' }
                } catch {
                    $removeFailure = "Affected-check dependency projection file cleanup failed: $($_.Exception.Message)"
                    $projectionCleanupError = if ($null -eq $projectionCleanupError) { $removeFailure } else { "$projectionCleanupError $removeFailure" }
                }
            }
        }
    }
    ...
    $aggregate = [pscustomobject][ordered]@{
        ...
        result=$result; failure_kind=if($result-ceq'pass'){$null}elseif(-not$child.Started){'launch'}elseif($child.TimedOut){'timeout'}elseif($child.OutputTruncated){'output-limit'}elseif($child.PostKillDrainTimedOut){'drain-timeout'}elseif(-not[string]::IsNullOrWhiteSpace([string]$child.Error)){'infrastructure'}else{'exit-code'}; exit_code=$child.ExitCode; timed_out=[bool]$child.TimedOut; output_truncated=[bool]$child.OutputTruncated; post_kill_drain_timed_out=[bool]$child.PostKillDrainTimedOut
...
foreach ($record in @($bindingRecords.ToArray())) {
    ...
    if ($blockedBy.Count -gt 0) {
        ...
        Assert-AffectedValidationEnvironmentEvidence -Evidence $blockedReceipt.environment -RequireProjection $false
    ...
    if ($null -ne $reusable) {
        ...
        Assert-AffectedValidationEnvironmentEvidence -Evidence $reusedReceipt.environment -RequireProjection $true
        ...
        $aggregate = [pscustomobject][ordered]@{check_id=[string]$check.check_id;command_path=[string]$check.command_path;command_blob_sha1=[string]$binding.command_blob_sha1;result='pass';failure_kind=$null;exit_code=0;timed_out=$false;output_truncated=$false;post_kill_drain_timed_out=$false;stdout_sha256=Get-AffectedValidationBytesHash $stdout;stderr_sha256=Get-AffectedValidationBytesHash $stderr;stdout_bytes=[long]$stdout.Length;stderr_bytes=[long]$stderr.Length}
    ...
    $executedReceipt = [pscustomobject][ordered]@{
        ...
        child=[pscustomobject][ordered]@{started=[bool]$execution.child.Started;exit_code=$execution.child.ExitCode;failure_kind=$checkResult.failure_kind;timed_out=[bool]$execution.child.TimedOut;output_truncated=[bool]$execution.child.OutputTruncated;post_kill_drain_timed_out=[bool]$execution.child.PostKillDrainTimedOut;stdout=[pscustomobject][ordered]@{path='stdout.bin';bytes=[long]$execution.stdout.Length;sha256=[string]$checkResult.stdout_sha256};stderr=[pscustomobject][ordered]@{path='stderr.bin';bytes=[long]$execution.stderr.Length;sha256=[string]$checkResult.stderr_sha256}}
        environment=$execution.environment
        artifacts=@($artifactReferences)
        reused_from=$null;claims=[pscustomobject][ordered]@{check_only=$true;candidate_admission=$false;acceptance_authority=$false;publication_authority=$false;device_used=$false}
    }
    Assert-AffectedValidationEnvironmentEvidence -Evidence $executedReceipt.environment -RequireProjection ([bool]$executedReceipt.environment.projected)
