...
function New-CorrectionFixture([string]$Name){
    ...
    Write-CorrectionJson (Join-Path $ws 'workspace.state.json') $state;[IO.File]::WriteAllText((Join-Path $ws 'iteration-events.jsonl'),"{`"event_id`":`"active-unit-started`",`"sequence`":1}`n",[Text.UTF8Encoding]::new($false))
...
try{
    ...
    $fault=New-CorrectionFixture 'artifact-repair';$faultOut=Join-Path $fault.workspace 'receipts\fault-correction.json';$interrupted=$false;try{Invoke-MorphospaceCorrectResolvedBlockerEvidence -WorkspaceRoot $fault.workspace -UnitId active-unit -RepoMapPath $fault.map -CorrectionReceipt $fault.input -OutPath $faultOut -Execute -FaultAfter after-event|Out-Null}catch{$interrupted=$true}
    Assert-Correction ($interrupted-and-not(Test-Path $faultOut)) 'artifact interruption did not stop before artifact'
