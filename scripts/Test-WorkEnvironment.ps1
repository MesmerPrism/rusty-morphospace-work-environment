...
if ($SelfTest) {
    ...
    foreach ($quickTest in @(
        ...
        [pscustomobject]@{ name = "workflow:blocked-supersession-terminal-validation"; script = "Test-BlockedSupersessionTerminalValidation.ps1"; detail = "Validated exact owner-authenticated blocked supersession failure history, legitimate later continuation, and fail-closed adversarial rejection." },
