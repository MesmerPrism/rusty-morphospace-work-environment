...
if ($SelfTest) {
    ...
    foreach ($quickTest in @(
        ...
        [pscustomobject]@{ name = "workflow:external-owner-authorization"; script = "Test-ExternalOwnerAuthorization.ps1"; arguments = @("-SelfTest"); detail = "Validated canonical RSA-PSS external-owner authorization with ephemeral keys, canonical UTC seconds, LF/CRLF framing, edited-comment rejection, validated LF-only output persistence, exact-evidence idempotence, and negative identity, duplicate, time, changed-evidence, signature, key, and strict-policy cases." },
        ...
        [pscustomobject]@{ name = "workflow:quest-file-manager-permission-adapter"; script = "Test-QuestFileManagerPermissionObservationAdapter.ps1"; detail = "Validated current QFM permission facts remain read-only, explicit, and non-semantic." },
        ...
        [pscustomobject]@{ name = "workflow:historical-validation-debt-phase-runner"; script = "Test-HistoricalValidationDebtPhaseRunner.ps1"; arguments = @("-SelfTest"); detail = "Validated create-new phase receipts and raw streams, finite timeout with child-tree cleanup, bounded diagnostics, and typed collision/cache/reuse failures." },
        [pscustomobject]@{ name = "workflow:historical-validation-debt-baseline"; script = "Test-HistoricalValidationDebtBaseline.ps1"; arguments = @("-SelfTest"); detail = "Validated phase-aware focused current-history capture, exact reusable baseline evidence, immutable signed historical-debt ratcheting, explicit debt-bearing current success, receipt binding, and damaged-set rejection." },
        ...
        [pscustomobject]@{ name = "workflow:recovered-proposal-continuation"; script = "Test-RecoveredProposalContinuation.ps1"; arguments = @("-SelfTest"); detail = "Validated writer-produced timestamp recovery, proposal retirement and fresh preparation while preserving caller bytes and HEAD." }
    )) {
        try {
