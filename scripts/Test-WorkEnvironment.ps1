...
if ($SelfTest) {
    ...
    foreach ($quickTest in @(
        ...
        [pscustomobject]@{ name = "workflow:validating-candidate-rematerialization"; script = "Test-ValidatingCandidateRematerialization.ps1"; arguments = @("-SelfTest"); detail = "Validated exact validating-candidate lineage rematerialization, stale-selector invalidation, torn-transition repair, replay, and no Git/device/remote mutation." }
    )) {
        try {
