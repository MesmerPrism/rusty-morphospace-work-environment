...
if ($SelfTest) {
    ...
    foreach ($quickTest in @(
        ...
        [pscustomobject]@{ name = "workflow:active-write-scope-amendment"; script = "Test-ActiveWriteScopeAmendment.ps1"; detail = "Validated additive project-bounded active feature-unit write-scope amendments, exact CAS, transaction ownership, and no Git/device/remote mutation." },
            ...
            [pscustomobject]@{ name = "workflow:history-archive-checkpoint"; script = "Test-HistoryArchiveValidation.ps1"; arguments = @("-SelfTest"); detail = "Validated additive raw-byte history archive roots, typed idle checkpoint/recovery, live-tail integrity, and replay-required damage routing without history, Git, device, or remote mutation." }
    )) {
        try {
