...
function Get-RepositoryIdentity { param([string]$Repository)
    ...
    if($root-cne$requested){throw 'Repository path must be the exact Git worktree root.'};Assert-SafePathChain $root $root
...
function Test-JsonWorkspace { param([string]$Workspace,[object[]]$Inventory)
    foreach($row in @($Inventory)){$path=Join-Path $Workspace ([string]$row.path)
        ...
        elseif([string]$row.path-match'\.jsonl$'){$count=0;foreach($line in [IO.File]::ReadLines($path)){$count++;if($count-gt$MaxJsonlRecords){throw 'JSONL record bound exceeded.'};if([Text.Encoding]::UTF8.GetByteCount($line)-gt$MaxJsonlLineBytes){throw 'JSONL line byte bound exceeded.'};if([string]::IsNullOrWhiteSpace($line)){throw 'Blank JSONL record.'};try{$null=$line|ConvertFrom-Json -Depth $MaxJsonDepth -ErrorAction Stop}catch{throw 'Invalid or over-depth JSONL record.'}}}
    }
}
...
try{$createdParents=@(New-OwnedParents $planning.root $parent);Assert-SafePathChain $planning.root $parent;$stage=Join-Path $parent ('.'+[IO.Path]::GetFileName($destination)+'.materializing-'+[guid]::NewGuid().ToString('N'));if([IO.Path]::GetPathRoot($parent)-cne[IO.Path]::GetPathRoot($stage)){throw 'Stage is not same-volume.'};[IO.Directory]::CreateDirectory($stage)|Out-Null;Assert-SafePathChain $planning.root $stage
