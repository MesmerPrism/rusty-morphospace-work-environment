...
function Test-MorphospaceAffectedValidationRegistry {
    ...
    foreach ($claim in @('shadow_only', 'executes_checks', 'skips_required_checks', 'acceptance_authority', 'publication_authority')) {
        if ($null -eq $Registry.claims.PSObject.Properties[$claim]) { throw "Affected-validation registry is missing claim '$claim'." }
    }
    if (-not [bool]$Registry.claims.shadow_only -or [bool]$Registry.claims.executes_checks -or [bool]$Registry.claims.skips_required_checks -or [bool]$Registry.claims.acceptance_authority -or [bool]$Registry.claims.publication_authority) {
        throw 'Affected-validation registry must remain shadow-only and non-authorizing.'
    }
    ...
    foreach ($check in @($Registry.checks)) {
        foreach ($id in @($check.prerequisite_checks)) { if (-not $checkMap.ContainsKey([string]$id)) { throw "Check '$($check.check_id)' references unknown prerequisite '$id'." } }
...
function Resolve-MorphospaceAffectedValidation {
    ...
    foreach ($check in @($registry.checks | Sort-Object check_id)) {
        ...
        if ($selectedReasons.ContainsKey($id)) {
            ...
            $skipped.Add([pscustomobject][ordered]@{ check_id=$id; platforms=@($check.platforms); minimum_tier=[string]$check.minimum_tier; reasons=@('shadow-plan-not-selected'); budget_seconds=[long]$check.budget_seconds; cache_policy=[string]$check.cache_policy })
        }
    }
    ...
    $planWithoutHash = [pscustomobject][ordered]@{
        ...
        claims = [pscustomobject][ordered]@{ shadow_only=$true; checks_executed=$false; required_checks_skipped=$false; acceptance_authority=$false; publication_authority=$false }
    }
