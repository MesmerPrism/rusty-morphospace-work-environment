...
function Invoke-MorphospaceAuthorityHostProbe {
    ...
    if(-not$HostPath){$hostCommand=Get-Command pwsh -CommandType Application -ErrorAction Stop|Where-Object{[IO.File]::Exists([string]$_.Source)}|Sort-Object -Property @{Expression={[version]$_.Version};Descending=$true},@{Expression={[string]$_.Source};Descending=$false}|Select-Object -First 1;if($null-eq$hostCommand){throw'PowerShell 7 child host is unavailable.'};$HostPath=[string]$hostCommand.Source}
...
function New-MorphospaceAuthorityPreflightV2 {
    param([Parameter(Mandatory=$true)][string]$ProjectId,[Parameter(Mandatory=$true)][string]$UnitId,[Parameter(Mandatory=$true)][string]$AttemptId,[Parameter(Mandatory=$true)][object]$ActionReference,[Parameter(Mandatory=$true)][object]$CapsuleReference,[Parameter(Mandatory=$true)][object]$HostReference,[Parameter(Mandatory=$true)][object]$RunnerReleaseReference,[Parameter(Mandatory=$true)][object]$ValidatorProbe,[Parameter(Mandatory=$true)][string]$CleanroomFingerprint,[Parameter(Mandatory=$true)][bool]$CapsuleReused)
    return [pscustomobject][ordered]@{schema='rusty.morphospace.workflow.authority_preflight_result.v2';preflight_id="$UnitId-$AttemptId-preflight";completed_at=[DateTime]::UtcNow.ToString('yyyy-MM-ddTHH:mm:ss.fffffffZ');project_id=$ProjectId;unit_id=$UnitId;attempt_id=$AttemptId;action=$ActionReference;capsule=$CapsuleReference;host_capabilities=$HostReference;runner_release=$RunnerReleaseReference;cleanroom_fingerprint_sha256=$CleanroomFingerprint;capsule_reused=$CapsuleReused;validator_probe=$ValidatorProbe;status='ready-for-record';does_not_prove=@('Admission only: does not execute acceptance commands or constitute owner validation evidence, validation execution, a validation receipt, acceptance, or an external-operation receipt.')}
}

function Test-MorphospaceAuthorityPreflightV2 {
    ...
    Assert-MorphospaceExactPropertySet $Preflight @('schema','preflight_id','completed_at','project_id','unit_id','attempt_id','action','capsule','host_capabilities','runner_release','cleanroom_fingerprint_sha256','capsule_reused','validator_probe','status','does_not_prove') @() 'authority preflight'
    ...
    if([string]$Preflight.schema-cne'rusty.morphospace.workflow.authority_preflight_result.v2'-or[string]$Preflight.status-cne'ready-for-record'-or[string]$Preflight.project_id-cne$ExpectedProjectId-or[string]$Preflight.unit_id-cne$ExpectedUnitId-or[string]$Preflight.attempt_id-cne$ExpectedAttemptId-or[string]$Preflight.preflight_id-cne"$ExpectedUnitId-$ExpectedAttemptId-preflight"-or-not(Test-MorphospaceStrictUtcTimestamp ([string]$Preflight.completed_at))-or[string]$Preflight.cleanroom_fingerprint_sha256-notmatch'^[0-9a-f]{64}$'-or@($Preflight.does_not_prove).Count-eq0){throw 'Authority preflight is not ready for record.'}
    Assert-MorphospaceExactPropertySet $Preflight.validator_probe @('validator_id','status','exit_code','probe_schema','probe_sha256','stdout_sha256','stderr_sha256') @() 'authority preflight validator probe';Assert-MorphospaceReadinessId ([string]$Preflight.validator_probe.validator_id) 'Preflight validator id'
    if([string]$Preflight.validator_probe.status-cne'pass'-or[int]$Preflight.validator_probe.exit_code-ne0-or[string]$Preflight.validator_probe.probe_schema-cne'rusty.morphospace.workflow.owner_validator_admission_probe.v1'){throw 'Authority preflight validator admission probe did not pass.'};foreach($field in @('probe_sha256','stdout_sha256','stderr_sha256')){if([string]$Preflight.validator_probe.$field-notmatch'^[0-9a-f]{64}$'){throw "Authority preflight validator probe $field is invalid."}}
...
Microsoft.PowerShell.Core\Export-ModuleMember -Function Get-MorphospaceReadinessSha256,Write-MorphospaceReadinessJsonNoOverwrite,New-MorphospaceAuthorityRunnerReleaseV1,Test-MorphospaceAuthorityRunnerReleaseV1,Invoke-MorphospaceAuthorityHostProbe,Test-MorphospaceAuthorityHostCapabilitiesV1,New-MorphospaceAuthorityInputCapsuleV1,Test-MorphospaceAuthorityInputCapsuleV1,New-MorphospaceAuthorityPreflightV2,Test-MorphospaceAuthorityPreflightV2,New-MorphospaceAuthorityReportContext,Write-MorphospaceAuthorityStageResult,Write-MorphospaceAuthorityFailureReport
