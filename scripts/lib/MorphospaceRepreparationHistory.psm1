...
function Assert-RepreparationHistoricalSuffix {
    ...
    $retirementReceiptPath=Get-RepreparationPath $Workspace ([string]$Recovery.retirement.receipt.path);$retirementReceiptProbe=Read-MorphospaceProtocolJson $retirementReceiptPath;$retirementReceiptSchemaName=switch -CaseSensitive ([string]$retirementReceiptProbe.schema){'rusty.morphospace.workflow.work_unit_automation_receipt.v1'{'work-unit-automation-receipt.schema.json'}'rusty.morphospace.workflow.proposed_unit_retirement_receipt.v1'{'proposed-unit-retirement-receipt-v1.schema.json'}default{throw "Repreparation retirement receipt schema '$([string]$retirementReceiptProbe.schema)' is unsupported."}}
    $retirementReceiptSnapshot=Read-RepreparationBoundSnapshot $Workspace $Recovery.retirement.receipt 'retirement receipt' $RepoRoot $retirementReceiptSchemaName 'Repreparation retirement receipt is invalid.'
    ...
    if([string]$retirementReceipt.schema-ceq'rusty.morphospace.workflow.proposed_unit_retirement_receipt.v1'){$transaction=$retirementReceipt.transaction;$pre=$retirementReceipt.proposed_retirement.authenticated_preimage;if([string]$transaction.transaction_id-cne$retirementTransaction-or[string]$transaction.state_path-cne[string]$retirementIntent.state.path-or[string]$transaction.unit_path-cne[string]$retirementIntent.unit.path-or[string]$transaction.events_path-cne[string]$retirementIntent.events.path-or[string]$transaction.receipt_path-cne[string]$Recovery.retirement.receipt.path-or[string]$transaction.event_id-cne[string]$retirementEvent.event_id-or[string]$transaction.expected_pre_state_sha256-cne[string]$retirementIntent.pre.state.sha256-or[string]$transaction.expected_pre_unit_sha256-cne[string]$retirementIntent.pre.unit.sha256-or[string]$transaction.expected_pre_unit_raw_sha256-cne[string]$pre.unit_raw_sha256-or[string]$transaction.expected_events_sha256-cne[string]$retirementIntent.expected.events_sha256-or[int64]$transaction.expected_events_length-ne[int64]$retirementIntent.expected.events_length-or[string]$transaction.expected_event_tail_id-cne[string]$retirementIntent.expected.event_tail_id){throw 'Repreparation narrow retirement transaction binding is detached.'}}
