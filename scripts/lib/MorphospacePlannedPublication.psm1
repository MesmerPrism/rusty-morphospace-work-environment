...
function Test-MorphospacePlannedPublicationDocument {
    ...
    $acceptedPartition=$null-ne$d.PSObject.Properties['accepted_unit_attribution']
    $acceptedPartitionIds=@()
    if($acceptedPartition){
      if([string]$d.accepted_unit_attribution.mode-cne'exact-prepared-range-partition' -or [string]$d.accepted_unit_attribution.trigger_unit_id-cne[string]$d.trigger_unit_id){throw 'Accepted-unit attribution marker or trigger binding is invalid.'}
      $acceptedPartitionIds=@($d.accepted_unit_attribution.unit_ids|ForEach-Object{[string]$_})
      if($acceptedPartitionIds.Count-lt2-or@($acceptedPartitionIds|Sort-Object -Unique).Count-ne$acceptedPartitionIds.Count-or$acceptedPartitionIds-notcontains[string]$d.trigger_unit_id){throw 'Accepted-unit attribution IDs must be unique and include the unchanged trigger.'}
    }
 ...
 foreach($r in @($d.repositories)){
  ...
  if(-not$published){
   ...
   $localSuffix=@(Invoke-PlannedPublicationGit $repo @('rev-list','--reverse',"$($r.final_revision)..$($s[0].head)") 'Planning prerequisite-suffix enumeration');if($localSuffix.Count-ne[int]$s[0].ahead){throw "Planning repository '$id' ahead count does not match its prerequisite suffix."}
    ...
    $recovery=$r.PSObject.Properties['intervening_accepted_publication'];if($null-ne$recovery){$historyMatches=@($r.commits|Where-Object{@($_.changed_paths|Where-Object{[string]$_-ceq$executedRelative}).Count-eq1});$intervening=@(Invoke-PlannedPublicationGit $repo @('rev-list','--reverse',"$($r.intervening_accepted_publication.executed_final_revision)..$($r.final_revision)") 'Recovered planning history enumeration');if($historyMatches.Count-ne1-or[string]$historyMatches[0].role-cne'intervening-planning-evidence'-or[string]$historyMatches[0].evidence_kind-cne'blocker'-or$null-ne$historyMatches[0].unit_id-or$intervening-notcontains[string]$historyMatches[0].revision){throw 'Recovered accounting-only prerequisite lacks one exact intervening executed-evidence commit.'};$actualEvidencePaths=@(Invoke-PlannedPublicationGit $repo @('diff-tree','--no-commit-id','--name-only','-r',[string]$historyMatches[0].revision) 'Recovered executed-evidence commit paths');if($actualEvidencePaths-notcontains$executedRelative){throw 'Recovered executed-evidence path is absent from its recorded commit.'};$required=@($accountingRelative)};$observed=@()
