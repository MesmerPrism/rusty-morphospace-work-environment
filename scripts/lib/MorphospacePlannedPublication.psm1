...
function Test-MorphospacePlannedPublicationDocument {
    param([Parameter(Mandatory)][string]$Path,[Parameter(Mandatory)][string]$WorkspaceRoot,[int]$ProvenanceDepth=0)
    if($ProvenanceDepth -gt 4){throw 'Synchronized carried-acceptance provenance is too deep or cyclic.'}
    ...
    foreach($r in @($d.repositories)){
      ...
      $synchronized=$null-ne$r.PSObject.Properties['synchronized_carried_acceptance']
      ...
      if(-not$synchronized){foreach($unitId in $seen.Keys){if(@($commitIds|Where-Object{$_-ceq$unitId}).Count-lt1){throw "Repository '$id' unit '$unitId' has no attributed commit."}}}
      ...
      elseif($synchronized){
        if(@($r.commits).Count-ne0-or[string]$r.old_revision-cne[string]$r.final_revision-or[string]$r.prepared_revision-cne[string]$r.final_revision-or[string]$r.synchronized_carried_acceptance.carried_revision-cne[string]$r.final_revision){throw "Synchronized source '$id' must be an exact zero-commit prepared/final/readback leg."}
        ...
        $priorPath=Test-PlannedPublicationBinding $r.synchronized_carried_acceptance.prior_accounting $WorkspaceRoot 'prior publication accounting';if((Resolve-Path -LiteralPath $priorPath).Path-ceq(Resolve-Path -LiteralPath $Path).Path){throw 'Prior publication accounting cannot reference itself.'};$prior=(Test-MorphospacePlannedPublicationDocument $priorPath $WorkspaceRoot ($ProvenanceDepth+1)).document
        if([string]$prior.project_id-cne[string]$d.project_id){throw 'Prior publication accounting project mismatch.'};$priorRepo=@($prior.repositories|Where-Object{$_.repo_id-ceq$id});if($priorRepo.Count-ne1-or$null-ne$priorRepo[0].PSObject.Properties['synchronized_carried_acceptance']){throw 'Prior accounting must contain one normal nonempty source range.'}
