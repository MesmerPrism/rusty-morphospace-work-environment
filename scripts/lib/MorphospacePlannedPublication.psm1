...
function Test-MorphospacePlannedPublicationDocument {
    ...
    foreach($r in @($d.repositories)){
      ...
      if(-not$synchronized-and-not$readback){foreach($unitId in $seen.Keys){if(@($commitIds|Where-Object{$_-ceq$unitId}).Count-lt1){throw "Repository '$id' unit '$unitId' has no attributed commit."}}}
      ...
      elseif($readback){
        if(@($r.commits).Count-ne0-or@($r.units).Count-ne1-or[string]$r.units[0].unit_id-cne[string]$d.trigger_unit_id-or$r.units[0].status_at_publication-ne'accepted'-or$r.units[0].no_acceptance_claim-ne$true){throw "Synchronized readback '$id' must bind one accepted triggering-unit status without inferring acceptance."}
        if([string]$r.old_revision-cne[string]$r.prepared_revision-or[string]$r.old_revision-cne[string]$r.final_revision-or[string]$r.old_revision-cne[string]$r.remote_readback_revision-or[string]$r.old_revision-cne[string]$r.synchronized_readback.unchanged_revision){throw "Synchronized readback '$id' must bind four equal revisions."}
