...
function Get-MorphospaceAuthorityObservation {
    param([object]$Unit,[hashtable]$RepositoryMap,[object]$Ownership,[string]$GitExecutable='',[object[]]$AutomationOutputs=@(),[string]$WorkspaceRoot='')
    ...
    $instructions=@(Get-MorphospaceInstructionObservation $Unit $RepositoryMap -WorkspaceRoot $WorkspaceRoot);$document=[pscustomobject][ordered]@{repositories=@($repositories.ToArray());instructions=$instructions};return [pscustomobject]@{document=$document;sha256=(Get-MorphospaceCanonicalJsonSha256 $document)}
}
...
function Test-MorphospaceUnitOwnership {
    param([object]$Ownership,[object]$ClaimBaseline,[object]$ClaimBaselineReference,[object]$Unit,[object]$RepositoryMapReference,[hashtable]$RepositoryMap,[switch]$BootstrapException,[string[]]$CommittedTransitionPaths=@(),[string]$WorkspaceRoot='')
    ...
    $finalObservation=Get-MorphospaceAuthorityObservation $Unit $RepositoryMap $Ownership -AutomationOutputs $automationOutputs -WorkspaceRoot $WorkspaceRoot;if([string]$finalObservation.sha256-cne[string]$observation.sha256){throw 'Repository/instruction state changed after terminal ownership probes.'};return [pscustomobject]@{observation=$finalObservation;ownership_by_repo=$ownershipRows;baseline_by_repo=$baselineRows;automation_outputs=$automationOutputs}
}
