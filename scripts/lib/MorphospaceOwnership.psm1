...
function Get-MorphospaceAuthorityObservation {
    param([object]$Unit,[hashtable]$RepositoryMap,[object]$Ownership,[string]$GitExecutable='')
...
function Test-MorphospaceUnitOwnership {
    ...
    if($ownershipRows.Count-ne$baselineRows.Count){throw 'Ownership repository set is incomplete.'};$observation=Get-MorphospaceAuthorityObservation $Unit $RepositoryMap $Ownership
    ...
    $gitExecutable=(Get-MorphospaceBoundExecutable git).path;foreach($repoObservation in @($observation.document.repositories)){$id=[string]$repoObservation.repo_id;$row=$ownershipRows[$id];$baselineContext=$baselineRows[$id];$baseline=$baselineContext.document;if((Get-MorphospaceCanonicalJsonSha256 $repoObservation)-cne[string]$row.live_content_observation_sha256){throw "Live content observation changed for '$id'."};if([string]$repoObservation.kind-ceq'git'){if([string]$repoObservation.head_revision-cne[string]$row.head_revision-or[string]$repoObservation.head_tree-cne[string]$row.head_tree_oid-or[string]$repoObservation.status_sha256-cne[string]$row.live_status_sha256-or[string]$repoObservation.overlay_fingerprint_sha256-cne[string]$row.live_overlay_fingerprint_sha256-or[string]$repoObservation.commit_fingerprint_sha256-cne[string]$row.live_commit_manifest_fingerprint_sha256){throw "Live Git identity/fingerprint changed for '$id'."}}
    ...
    $finalObservation=Get-MorphospaceAuthorityObservation $Unit $RepositoryMap $Ownership;if([string]$finalObservation.sha256-cne[string]$observation.sha256){throw 'Repository/instruction state changed after terminal ownership probes.'};return [pscustomobject]@{observation=$finalObservation;ownership_by_repo=$ownershipRows;baseline_by_repo=$baselineRows}
}
