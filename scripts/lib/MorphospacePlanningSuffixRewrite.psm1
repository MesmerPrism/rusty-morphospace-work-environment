...
function Test-MorphospacePlanningSuffixRewriteLive {
    ...
    $common=@($rp.common_changed_paths|ForEach-Object{[string]$_});if($common.Count-ne2-or@($common|Sort-Object -Unique).Count-ne2){throw 'Planning suffix common changed-path set must contain exactly two unique paths.'}
    foreach($revision in @($rp.first_suffix_revision,$rp.replacement_suffix_revision)){$paths=@(Invoke-PlanningSuffixGit $repo @('diff-tree','--no-commit-id','--name-only','-r',[string]$revision)|ForEach-Object{[string]$_}|Sort-Object);if(($paths-join'|')-cne(@($common|Sort-Object)-join'|')){throw 'Planning suffix does not match the exact common parent-relative changed-path set.'}}
    $delta=@($rp.replacement_delta_paths|ForEach-Object{[string]$_});if($delta.Count-ne1-or@($delta|Sort-Object -Unique).Count-ne1-or$common-notcontains[string]$delta[0]){throw 'Planning replacement delta must contain exactly one path from the common suffix set.'}
    $actualDelta=@(Invoke-PlanningSuffixGit $repo @('diff-tree','--no-commit-id','--name-only','-r',[string]$rp.first_suffix_revision,[string]$rp.replacement_suffix_revision)|ForEach-Object{[string]$_}|Sort-Object);if(($actualDelta-join'|')-cne(@($delta|Sort-Object)-join'|')){throw 'Planning replacement tree delta does not match the exact bound path set.'}
    $v
}
