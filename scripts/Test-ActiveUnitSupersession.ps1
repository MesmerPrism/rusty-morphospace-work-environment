...
try{
    ...
    $orderingPaths=@(
        ...
        'apps/example/native/renderer.rs',
        'src/feature.rs',
        'tools/Invoke-Example.ps1',
        'tools/checks/Test-Example.ps1'
    )
    $orderingExpected=[string[]]@($orderingPaths)
    [Array]::Sort($orderingExpected,[StringComparer]::Ordinal)
    $priorCulture=[Threading.Thread]::CurrentThread.CurrentCulture
    try{
        [Threading.Thread]::CurrentThread.CurrentCulture=[Globalization.CultureInfo]::GetCultureInfo('en-DE')
        $linguisticOrder=@($orderingPaths|Sort-Object -CaseSensitive)
        Assert-ActiveTest (($linguisticOrder-join[char]10)-cne($orderingExpected-join[char]10)) 'culture regression fixture does not distinguish linguistic and ordinal path order'
        $orderingScope=@([pscustomobject]@{repo_id='source-repo';allowed_paths=@($orderingPaths)})
        $orderingOld=[pscustomobject]@{allowed_repositories=$orderingScope}
        $orderingReplacement=[pscustomobject]@{allowed_repositories=$orderingScope}
        $orderingMap=(Invoke-ActivePrivate {param($a)Get-ActiveSupersessionRepositoryMap @a} @($templateRepoMap)).map
        $orderingRows=@(Invoke-ActivePrivate {param($a)Get-ActiveSupersessionRepositoryObservation @a} @($orderingOld,@([pscustomobject]@{unit_id='unit-new';role='replacement';document=$orderingReplacement}),$orderingMap))
        Assert-ActiveTest ($orderingRows.Count-eq1-and(@($orderingRows[0].allowed_paths)-join[char]10)-ceq($orderingExpected-join[char]10)) 'repository observation did not emit canonical ordinal allowed paths under en-DE culture'
    }finally{
