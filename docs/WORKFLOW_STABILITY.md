...
Use `!cancelled()` on the required job or step guards that must remain eligible after
...
validation graph can stop. Main segment and delta job conditions retain implicit
